...
<section class="full">
 ...
 $query = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[business_trip] WHERE request_id='".$_GET['id']."'");
      ...
      $query4 = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[users] WHERE employee_no='".$user['admin']."'");
       ...
       foreach($query4 as $item4){
        ...
        $query5 = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[users] WHERE employee_no='".$user['parent2']."'");
...
<hr>
    ...
    <form id="popup_form" method="post" enctype="multipart/form-data">
      ...
      <input type="hidden" name="request" value="business-trip-request-response2-parent2"/>
      ...
      <label class="radio">
        <input type="radio" name="status_parent2" value="1" checked="true">
        <i class="ion-android-checkbox-outline"></i><br>
        <?php echo __('Odobreno'); ?>
      </label>
      <label class="radio">
        <input type="radio" name="status_parent2" value="2">
	  ...
	  <?php } ?>
    <?php if($row['comment_parent2'] != null) { ?>
      <small><?php echo __('Komentar od '.' '.$parent2_f.' '.$parent2_l.':'); ?></small>
      <b><?php echo $row['comment_parent2']; ?></b><br/>
    <?php } ?>
      ...
      <textarea name="comment_parent2" style="margin:10px!important;width:98%;"></textarea>

      <hr/>

      <button type="submit" style="margin-bottom:20px;width:200px;" class="btn btn-red pull-right" style="margin-bottom:20px;width:200px;margin-right:15px;"><?php echo __('Spasi!'); ?> <i class="ion-ios-download-outline"></i></button>
