...
if (isset($_POST['request'])) {
   ...
   and  [year]='" . $absence_year . "'");
    ...
    if ($_POST['request'] == 'parent-day-add_apsolute' or ($_POST['request'] == 'day-edit' and $_POST['status'] == 67)) {
        ...
        if ((($weekends_start >= 6 and $weekends_start <= 7) and ($weekends_end >= 6 and $weekends_end <= 7)) and $_user['employee_no'] != $_SESSION['cc_admin'] and !in_array($status, array(43, 44, 45, 61, 62, 65, 67, 68, 69, 74, 75, 76, 77, 78, 73, 81, 85, 86, 87, 88, 89, 90, 105, 107, 108))) {
  ...
  AND weekday<>'6' AND weekday<>'7' AND (corr_status='19' or corr_status='18') AND (status <> corr_status)");
...
//Upis pocetka bolovanja
  ...
  AND weekday<>'6' AND weekday<>'7' AND (corr_status='32')");
     ...
     and weekday<>'6' AND weekday<>'7' AND (corr_status='79')");
        ...
        if ((in_array($_POST['status'], array(85, 86, 87, 88, 89, 90)) or ($_POST['hour_pre'] != '' and $_POST['hour_pre'] != '') or ($_POST['hour'] <> 8 and $br_sati == 8)) or ($old_status['weekday'] == '6' or $old_status['weekday'] == '7')) {
   ...
   and year_id=?
    ";
	  ...
	  corr_review_status = case when ([KindOfDay]='BHOLIDAY' or Description<>'' or status_bh is not null) then 1 else 0 end,
   ...
   and year_id=?
    ...
    if ($_POST['request'] == 'day-review') {
        ...
        $check = $db->query("SELECT user_id, year_id,month_id,employee_no,weekday FROM  " . $portal_hourlyrate_day . "  WHERE id='" . $this_id . "' ");
        foreach ($check as $checkvalue) {
	  ...
	  corr_status = case when ([KindOfDay]='BHOLIDAY' or Description<>'' or status_bh is not null) then 83 else 5 end
	 WHERE id = ?
	 AND (KindOfDay<>'BHOLIDAY' or corr_review_status=0)";
