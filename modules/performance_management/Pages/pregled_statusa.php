...
try{
    ...
    foreach($excelQuery as $user){
        $just_user = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[users] WHERE user_id = ".$user['user_id'])->fetch();

        $sporazum = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[pm_sporazumi] where user_id = ".$just_user['user_id']." and year = ".date('Y'))->fetch();

        $goodToGo = false;
        foreach($children as $child){
            if($just_user['user_id'] == $child) $goodToGo = true;
        }
        if(!$sporazum) {
            if ($pm_admin or $goodToGo) {
                $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A'.$counter, $just_user['employee_no'])
                    ->setCellValue('B'.$counter, $just_user['dosier_no'])
                    ->setCellValue('C'.$counter, $just_user['fname'].' '.$just_user['lname']);
            }
            $counter++;
        }
    }
...
<div class="split-on-right">
    <div class="choose-what-to-do choose-what-to-do-full">
        ...
        <p>
            <a href="modules/performance_management/pages/pregled_statusa.xls" download> Izvještaj.xls </a>
            <a href="modules/performance_management/pages/pregled_statusa.xlsx" download> Izvještaj.xlsx </a>
        </p>
        <table>
            <thead>
                <tr>
            	...
            	foreach($users as $user){
            		...
            		if(!$sporazum){
            			if($pm_admin or $goodToGo){
                            ?>
                            <tr>
                                <td><?php echo $just_user['employee_no']; ?></td>
                                <td><?php echo $just_user['dosier_no']; ?></td>
                                <td><?php echo $just_user['fname'].' '.$just_user['lname']; ?></td>
                            </tr>
                            <?php
                        }
            		}
        ...
        <div class="btn-group paginate">
            <?php echo _pagination('?m=performance_management&p=pregled_statusa&page=', $page, '20', $totalRows - 20); ?>
        </div>
    </div>
</div>