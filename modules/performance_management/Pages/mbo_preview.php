...
try{
    ...
    if($agreement['locked']) $brojKomentara = false;
...
if(($isSupervisor and !$agreement['accepted_from_supervisor'] and !$agreement['accepted_from_employee']  and $agreement['sent']) or ($owner and !$agreement['accepted_from_supervisor']) or ($evoluacija and $agreement['unlocked'] and $owner and !$agreement['accepted_from_supervisor']) or($evoluacija and $agreement['unlocked'] and $isSupervisor and !$agreement['accepted_from_employee']) or $pm_admin){
    $edit = true;
}

if($agreement['locked']) $edit = false;

$agreement = $agree->getAgreementById($_GET['preview']);

$newAgreement = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[pm_sporazumi] where id=".$_GET['preview'])->fetch();
?>
...
<div class="split-on-right">
    <div class="choose-what-to-do">
        ...
        foreach($goals as $goal){
            if(!$goal['disabled']){
                ...
                <div class="single-element">
                    ...
                    <?php
                    if($evoluacija and $owner and !$goal['disabled'] and !$agreement['status'] and $brojKomentara){
                        ?>
        ...
        <?php
        if($evoluacija and $newAgreement['accepted_from_supervisor'] and $newAgreement['accepted_from_employee']){
            ?>
            <form method="post">
                <div class="single-element">
                    <div class="just-a-row">
                            ...
                            <select name="recomended_grade" <?php if(!$isSupervisor and !$agreement['status']) echo 'disabled'; ?>>
                                ...
                                foreach($ocjene as $ocjena){
                                    if($ocjena['value'] > $agreement['final_grade']){
                                        ?>
                                    <option value="<?php echo $ocjena['value']; ?>" <?php if($ocjena['value'] == $newAgreement['recomended_grade'])  echo "selected"; ?>><?php echo $ocjena['name']; ?></option>
                                    <?php
                                    }
                                }
                                ?>
                    ...
                    <div class="just-a-row">
                        <div class="inside-col">
                            ...
                            <textarea name="supervisor_comm" id="" cols="30" rows="10" <?php if(!$isSupervisor and !$newAgreement['status']) echo 'readonly'; ?>><?php echo $newAgreement['supervisor_comm']; ?></textarea>
                        </div>
                    </div>

                    <?php
                    if($isSupervisor  and $newAgreement['status']){
                        ?>
                        <div class="just-a-row">
                            <div class="save-button">
                                <input type="submit" value="SPREMITE PODATKE">
        ...
        if($isSupervisor and !$newAgreement['accepted_from_supervisor'] and $newAgreement['sent'] and !$newAgreement['locked']){
            ?>
        ...
        if(!$newAgreement['sent'] and $owner and !$newAgreement['locked']){
            ?>
            <div class="classic-button-wrapper">
        ...
        if($owner and $newAgreement['accepted_from_supervisor'] and !$newAgreement['accepted_from_employee'] and $newAgreement['sent'] and !$newAgreement['locked']){
            ?>
            <div class="classic-button-wrapper">
        ...
        if($owner and $newAgreement['accepted_from_supervisor'] and $newAgreement['accepted_from_employee'] and $newAgreement['sent'] and $date_is_fine and !$newAgreement['locked']){
            ?>
            <div class="classic-button-wrapper">
        ...
        if($newAgreement['status']){ // Ako je poslano, znači da je sve izračunato
            ?>
            <div class="single-element">
                <div class="just-a-row">
                    <div class="inside-col">
        ...
        if($owner and $evoluacija and !$newAgreement['status'] and !$newAgreement['locked']){
            ?>
