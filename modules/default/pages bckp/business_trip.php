...
<section class="full">
 ...
 <a href="<?php echo $url.'/modules/'.$_mod.'/pages/popup_business_trip_add.php'; ?>" data-widget="ajax" data-id="opt2" class="btn btn-red btn-lg"><?php echo __('Dodaj službeno putovanje'); ?> <i class="ion-ios-plus-empty"></i></a>
        </div>
      </div>
    ...
    <a href="/app/?m=default&p=business_trip" class="btn btn-filter <?php if($type == ''){ echo 'active'; } ?>"><?php echo __('Aktivni'); ?></a>
    <a href="/app/?m=default&p=business_trip&t=3" class="btn btn-filter <?php if($type == '3'){ echo 'active'; } ?>"><?php echo __('Arhiva'); ?></a>
        ...
        foreach($query as $item){
		 ...
		 if ($item['country_ino'] != 1) {
		  ...
		  /*
           ...
           if( ($item['status_parent2']==0) and ($item['status_hr']==0 )){
            $border = 'blue';
          }elseif(($item['status_parent2']==1) and ($item['status_hr']==0)){
            $border = 'blue';
          }elseif(($item['status_parent2']==0) and ($item['status_hr']==1)){
            $border = 'blue';
          }elseif(($item['status_parent2']==1) and ($item['status_hr']==1)){
            $border = 'green';
          }elseif(($item['status_parent2']==2) or ($item['status_hr']==2)){
            $border = 'red';
          }elseif(($item['status_parent2']==3) or ($item['status_hr']==3)){
            $border = 'gray';
          }
      ...
      <div class="content">
        <div class="row">
          <div class="col-sm-4">
            ...
            <?php echo __('Do:'); ?> <b><?php echo date('d/m/Y',strtotime($item['h_to'])); ?></b><br/>
          ...
          <div class="col-sm-4">
            ...
            <?php }else if($item['status_parent2']==2){ ?>
              &nbsp; &nbsp; <span style="color:#990000;"><i class="ion-android-close"></i> <?php echo __('Odbio/la').' '.$parent2_f.' '.$parent2_l.' '."---".' '.date('d/m/Y H:i',strtotime($item['date_response']));  ?></span><br>
            <?php } ?>
            ...
            <?php if($item['status_user_edit']==1){ ?>
              &nbsp; &nbsp; <span style="color:#009900;"><i class="ion-android-checkmark-circle"></i> <?php echo __('Editovao/la').' '.$_user['fname'].' '.$_user['lname'].' '."---".' '.date('d/m/Y H:i',strtotime($item['date_user_edit'])); ?></span><br>
            <?php }else if($item['status_user_edit']==2){ ?>
              &nbsp; &nbsp; <span style="color:#990000;"><i class="ion-android-close"></i> <?php echo __('Odbio/la').' '.$_user['fname'].' '.$_user['lname'].' '."---".' '.date('d/m/Y',strtotime($item['date_user_edit'])); ?></span><br>
            <?php } ?>
            ...
            <?php }else if($item['status_parent_edit']==2){ ?>
              &nbsp; &nbsp; <span style="color:#990000;"><i class="ion-android-close"></i> <?php echo __('Odbio/la').' '.$_user['fname'].' '.$_user['lname'].' '."---".' '.date('d/m/Y',strtotime($item['date_parent_edit'])); ?></span><br>
            <?php } ?>

             <?php if($item['status_hr_edit']==1){ ?>
              &nbsp; &nbsp; <span style="color:#009900;"><i class="ion-android-checkmark-circle"></i> <?php echo __('Editovao/la').' '.$hr_f.' '.$hr_l.' '."---".' '.date('d/m/Y H:i',strtotime($item['date_hr_edit'])); ?></span><br>
          ...
          <div class="col-sm-2 text-right">
            <?php if( $item['lock']!='N' and $item['status']!=2 and ($item['status_admin_response']!=2) and ($item['status_admin2_response']!=2) ){ ?>
             <!--  <a href="<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>" class="table-btn alt" data-widget="remove" data-id="requests_remove:<?php echo $tools_id; ?>" data-text="<?php echo __('Dali ste sigurni da želite poništiti zahtjev?'); ?>"><i class="ion-android-close"></i></a> -->
              <a href="<?php echo '/app/?m=users&p=popup_business_trip_add_view&id='.$tools_id; ?>"class="table-btn alt" title="Pregled"><i class="ion-eye"></i></a>
              <a href="<?php echo '/app/?m=users&p=popup_business_trip_add_edit_user&id='.$tools_id; ?>"class="table-btn alt" title="Izmjene"><i class="ion-edit"></i></a>
            <?php }else{ ?>
