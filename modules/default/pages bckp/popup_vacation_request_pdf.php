<?php
	 ...
	 //new
	  ...
	  $check3 = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  

	  	where status!=83 and employee_no='$_user[employee_no]'   and Date between '".$data[0]['h_from']."' and '".$data[0]['h_to']."' and weekday not in (6,7)");
	  $data[3]= $check3->fetch();

	  $update_requests = $db->query("UPDATE $database_used.[requests] SET duration = ".$data[3][0]." where request_id = ".$data[0]['request_id' ]);
