...
?>
  ...
  <div class="container-fluid">
	 ...
	 ?>
			 ...
			 <span class="potvrda" style="margin-left:15px;margin-right:15px;">Zadužnica potpisana!</span><br/>
			 <?php }?>
			 
			   <?php if($radnik_odbio_zaduznica){?>
			 <span class="potvrda" style="margin-left:15px;margin-right:15px;">Zadužnica odbijena!</span><br/>
			 <?php }?>
	...
	<th style="height:20px;width: 60px;" >Radnik saglasan</th> 
			...
			<?php
				...
				<td><input readonly="readonly" type="text" class="rcorners1" style="width:60px;" name="zaduzeno[]" value="<?php echo _transformZaduzeno($zaduznice_statusi[$key]['zaduzen']); ?>"></td>
                 <td><input readonly="readonly" type="text" class="rcorners1" name="datum[]" value="<?php if($zaduznice_statusi[$key]['datum']=='') {echo '';} else {echo date('d/m/Y',strtotime($zaduznice_statusi[$key]['datum']));} ?>"></td>
				 <td><input readonly="readonly" type="text" class="rcorners1" name="odgovorna_osoba[]" value="<?php echo _user($zaduznice_statusi[$key]['odgovorna_osoba'])['fname'].' '._user($zaduznice_statusi[$key]['odgovorna_osoba'])['lname']; ?>"></td>
				<td><textarea  id="<?php echo 'zapisnik-'.$item['item_id'].'-'.$usr; ?>" readonly="readonly" name="zapisnik[]" <?php if($zaduznice_statusi[$key]['saglasan']==0) echo 'style="border-color: red"'; ?> rows="3" cols="40" spellcheck="false"><?php echo $zaduznice_statusi[$key]['zapisnik']; ?></textarea></td>
	...
	<a id="link_potvrda" href="<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>" style="float:right;display:<?php if($poslano_radniku_zaduznica and !$radnik_potpisao_zaduznica) {echo '';}else{echo 'none';} ?>" class="table-btn alt1" data-widget="send" data-id="radnik_potpisuje_zaduznica:<?php echo $usr;?>" data-text="<?php echo __('Da li ste sigurni da želite potpisati?'); ?>" data-response="<?php echo __('Uspješno potpisano!'); ?>"><i class="fa fa-arrow-circle-right" style="float:left;color:green;"></i><label style = "font-size:17px;cursor: pointer;">Potpiši</label></a><br><br><br>
		<a id="link_potvrda" href="<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>" style="float:right;display:<?php if($poslano_radniku_zaduznica and !$radnik_potpisao_zaduznica) {echo '';}else{echo 'none';} ?>" class="table-btn alt1" data-widget="send" data-id="radnik_odbija_zaduznica:<?php echo $usr;?>" data-text="<?php echo __('Da li ste sigurni da želite odbiti zadužnicu?'); ?>" data-response="<?php echo __('Uspješno odbijeno!'); ?>"><i class="fa fa-arrow-circle-right" style="float:left;color:green;"></i><label style = "font-size:17px;cursor: pointer;">Odbij zadužnicu</label></a><br><br><br>

			   </div>
			   
			     <button type="submit" style="margin-bottom:40px;width:200px;float:right;height:45px;margin-right:15px;" class="btn btn-red pull-right"><?php echo __('Printaj!'); ?> <i class="ion-ios-copy-outline"></i></button>
	        </form>
	</div>
 ...
 <script>
    ...
    return regex.test(jQuery(elem)[attr.method](attr.property));
		...
		$('select:regex(id, .*saglasan.*)').on('change', function() {
				$.post("<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>", { request: "change-saglasan", saglasan : this.value, saglasan_id : this.id}, 
	...
	$('textarea:regex(id, .*zapisnik.*)').on('change', function() {
				...
				$.post("<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>", { request: "change-zaduznica-zapisnik", zapisnik : this.value, zapisnik_id : this.id}, 
    function(returnedData){
