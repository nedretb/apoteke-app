  ...
  <div class="content clear">
    ...
    <?php if($row['is_accepted']==0){ ?>
      <div class="text-center">
        <?php echo __('Zadatak jos nije prihvaćen od stane nadređenog, i zbog toga nije moguće obraditi stavke zadatka.'); ?><br/>
    ...
    <form id="popup_form" method="post" enctype="multipart/form-data">
      ...
      <button type="submit" class="btn btn-red pull-right"><?php echo __('Dodaj komentar!'); ?> <i class="ion-ios-download-outline"></i></button>
    ...
    $(function(){
      ...
      $('[data-widget="completed"]').on("click",function(e){
        ...
        var result  = $(this).attr('data-id').split(':');
        var rm    = result[1];
        $.post(_url, { request: 'completed-'+result[0], request_id: rm }, function(data){ window.location.reload(); });
      });
      ...
      $('.processing').on('click','input[name=items]',function(){
          ...
          if($(this).is(':checked')){
            $.post('<?php echo $url.'/modules/default/ajax.php'; ?>', { request: 'proc-tasks', request_id: _id, status: '1' }, function(data){
              $('#ch'+_id).wrap('<strike></strike>');
              $.post('<?php echo $url.'/modules/default/ajax.php'; ?>', { request: 'count-tasks', request_id: '<?php echo $_GET["id"]; ?>' }, function(data){
                if(data=='yes'){
                  $('#completed').slideDown();
                }
              });
            ...
            $.post('<?php echo $url.'/modules/default/ajax.php'; ?>', { request: 'proc-tasks', request_id: _id, status: '0' }, function(data){
              $('#ch'+_id).unwrap();
              $.post('<?php echo $url.'/modules/default/ajax.php'; ?>', { request: 'count-tasks', request_id: '<?php echo $_GET["id"]; ?>' }, function(data){
                if(data=='no'){
                  $('#completed').slideUp();
      ...
      function _loadComments(){
       ...
       var _parent = '<?php echo _employee($row['parent_id'])['user_id']; ?>';
        $.post('<?php echo $url.'/modules/default/ajax.php'; ?>', { request: 'comments', request_id: _id, user: _user, parent: _parent }, function(data){ $('#comments').html(data); });
      }
      ...
      $('form').on('click','.btn-sr',function(e){
        ...
        $.post('<?php echo $url.'/modules/default/ajax.php'; ?>', { request: 'task-review-item', data: _ser }, function(data){
          if(data){
            $('.dialog-loader').hide();
