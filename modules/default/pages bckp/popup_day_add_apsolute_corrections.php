  ...
  $get_m = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_month."  WHERE id='".$_GET['month']."'");
    ...
    $getrow = $db->query("SELECT month_id FROM  ".$portal_hourlyrate_day." ");
      if($get->rowCount()<0){
		...
		$number_of_days = cal_days_in_month(CAL_GREGORIAN, $month['id'], $year['year']);
		
		 $getrow = $db->query("SELECT TOP 1 * FROM  ".$portal_hourlyrate_day."  WHERE month_id='".$_GET['month']."' and (weekday not in (6,7))  and user_id = ".$_user['user_id']);
      if($get->rowCount()<0){
        $row = $getrow->fetch();
    ...
    <form id="popup_form" method="post">
      ...
      <input type="hidden" name="request" value="parent-day-add_apsolute_corrections"/>
      ...
      <div class="row">
        <div class="col-sm-6">
         ...
         <div id="dt">
        <input type="text" name="dateFrom" class="form-control" style="width:120px;" id="dateOD" value="01.<?php echo $_GET['month']; ?>.<?php echo $year['year']; ?>" placeholder="dd/mm/yyyy" title="" required>
      </div><br/>
        </div>
        ...
        <div class="col-sm-6">
          ...
          <div id="dt">
        <input type="text" name="dateTo" class="form-control" style="width:120px;" id="dateDO" value="01.<?php echo $_GET['month']; ?>.<?php echo $year['year']; ?>" placeholder="dd/mm/yyyy" title="" required>
      </div><br/>
        </div>
      </div>
       ...
       <div class="row">
        <div class="col-sm-6">
          ...
          <input type="number" name="hour" value="<?php echo $br_sati; ?>" min="0" max="24" class="form-control" readonly>
        </div>
...
?>
	...
	$('#komentar').prop('required',true);
		...
		$('#dateOD').datepicker({
			...
			language: 'bs',
  			startDate: startDate,
			endDate: new Date(year + '/'+'<?php echo $month['id']; ?>'+'/'+'<?php echo $number_of_days; ?>')
