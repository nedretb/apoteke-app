...
if($total>0 || $total2>0){
    ...
    <?php if($edit->rowCount()<0){
        foreach($edit as $valueedit) {$visible=$valueedit['editable'];}
    }
    else
        $visible='N';
    ...
    <section class="full">
                ...
                <div class="col-sm-9"><br/>
                    <div class="pull-right">
                        <?php if(($visible!='N')) {?>
                            ...
                            <a href="<?php echo $url.'/modules/'.$_mod.'/pages/popup_day_add_apsolute.php?year='.$year['id'].'&month='.$month['id']; ?>" data-widget="ajax" data-id="opt2" data-width="500" class="btn btn-red btn-lg"><?php echo __('Ažuriraj satnice'); ?> <i class="ion-ios-plus-empty"></i></a>
                            <a href="<?php echo $url.'/modules/'.$_mod.'/pages/popup_day_cancel_apsolute.php?year='.$year['id'].'&month='.$month['id']; ?>" data-widget="ajax" data-id="opt2" data-width="500" class="btn btn-red btn-lg"><?php echo __('Otkaži registraciju'); ?> <i class="ion-ios-plus-empty"></i></a>
                        <?php } {?>
        ...
        if(($day['review_status'] != '0' and ($day['KindofDay']!='BHOLIDAY' or $day['review_user']!='') ) and (($termination['termination_date']=='' or ($termination['termination_date']!='' and $day['Date'] <= $termination['termination_date'])) and ($day['Date'] >=$termination['employment_date']))){
            ...
            if(($day['weekday'] == '6' or $day['weekday'] == '7') and isAllowedStatusWeekend($day['status']) == false){
                ...
                echo '<a href="'.$url.'/modules/'.$_mod.'/pages/popup_day_view.php?id='.$day['id'].'" class="table-btn" data-widget="ajax" data-id="opt2" data-width="400"><i class="ion-eye"></i></a>';
            }
        ...
        if(($day['review_status']=='0') and ($day['weekday']!='6') and ($day['weekday']!='7') and
            ...
            echo '<a href="'.$url.'/modules/'.$_mod.'/pages/popup_day_edit.php?id='.$day['id'].'" class="table-btn" data-widget="ajax" data-id="opt2" data-width="400"><i class="ion-edit"></i></a>';
        }
        echo '</div>';
        echo '</div>';
        }
