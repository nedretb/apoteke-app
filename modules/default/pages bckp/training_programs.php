...
<section class="full tasks-page">
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-sm-12" style = "margin-bottom:10px;">
        <div class="box">
            ...
            <img style="width:240px;"  src="<?php echo $_uploadUrl; ?>/<?php echo _settings('logo'); ?>"><br/><br/>
			...
			<div class="btn btn-filter <?php if($faza == '1'){ echo 'active-task'; } ?>" style="margin-left:15px;margin-bottom: 15px;width: 130px;margin-right:7px;<?php if (!isset($obrazac_1)){ echo 'display:none;'; } ?>">  <a style="color:black" class="<?php if($faza == '1'){ echo 'active-white'; } ?>" href="?m=default&amp;p=training_programs&f=1">Plan  i program</a></div> <a style="color:black" href="?m=default&amp;p=training_programs&f=2"><i class="fa fa-arrow-right" style="position: absolute;margin-top: 9px;" aria-hidden="true"></i></a>
			<div class="btn btn-filter <?php if($faza == '2'){ echo 'active-task'; } ?>" style="margin-left:15px;margin-bottom: 15px;width: 80px;margin-right:7px;<?php if (!isset($obrazac_2)){ echo 'display:none;'; } ?>"> <a style="color:black" class="<?php if($faza == '2'){ echo 'active-white'; } ?>" href="?m=default&amp;p=training_programs&f=2">Test</a></div> <a style="color:black" href="?m=default&amp;p=training_programs&f=4"><i class="fa fa-arrow-right" style="position: absolute;margin-top: 9px;" aria-hidden="true"></i></a> 
			<div class="btn btn-filter <?php if($faza == '4'){ echo 'active-task'; } ?>" style="margin-bottom: 15px;width: 147px;margin-left:20px;margin-right: 15px;<?php if (!isset($obrazac_4)){ echo 'display:none;'; } ?>"> <a style="color:black" class="<?php if($faza == '4'){ echo 'active-white'; } ?>" href="?m=default&amp;p=training_programs&f=4">Evaluacija mentora</a></div>	
			</div>
	  ...
	  $broj_status_mentor =$broj_status_mentor1['broj'];
		   ...
		   <?php if($potpisao_mentor){?>
			 <span class="potvrda" style="margin-left:15px;margin-right:15px;margin-top:0px;">Obrazac potpisao mentor!</span><br/>
			 <?php }?>
		   
		   <?php if($potpisao_radnik){?>
			 <span class="potvrda" style="margin-left:15px;margin-top:-10px;margin-right:15px;">Obrazac potpisao radnik!</span><br/>
			 <?php }?>
    ...
    <th style="height:20px;width: 200px;" ></th> 
	 	   ...
	 	   <tr>
    <th style="height:20px; background: #c7bebe;color: black;width: 500px;">Tačke za ocjenu zaposlenika </th>
	...
	<th style="height:20px; background: #c7bebe;color: black;width: 200px;" >Ocjena mentora</th> 
	 ...
	 <?php }?>
			 ...
			 <tr>
				...
				<td><?php echo $desc; ?></td>
				 <td>	<select id="status" name="<?php echo $item['item_id'].'-'.$usr.'-'.$faza; ?>" class="rcorners1" class="form-control" style="width:140px;outline:none;"  >
     <?php echo _optionObukaStatus($training_program_statusi[$key]['status']); ?>
      </select></td>
                <td><textarea id="komentar" name="<?php echo $item['item_id'].'-'.$usr.'-'.$faza; ?>" rows="3" cols="40" spellcheck="false"><?php echo $training_program_statusi[$key]['komentar']; ?></textarea></td>
				<?php }?>
				 <?php if ($faza==1 and $obrazac=='b1_b2_dir_regije_po_centrala'){?>
     ...
     <?php echo _optionObukaStatus($training_program_statusi[$key]['status']); ?>
				...
				<td><?php echo $item['item_tema']; ?></td>
				<td><?php echo $desc; ?></td>
				 <td>	<select id="status" name="<?php echo $item['item_id'].'-'.$usr.'-'.$faza; ?>" class="rcorners1" class="form-control" style="width:180px;outline:none;"  >
     <?php echo _optionObukaTestStatus($training_program_statusi[$key]['status']); ?>
				...
				<?php }?>
					<?php if ($faza==4){?>	
			<td><?php echo $item['item_tema']; ?></td>					
				<td <?php if ($key==$broj_status_mentor-1){echo 'colspan="2"';}?>><textarea id="coment" name="<?php echo $item['item_id'].'-'.$usr.'-'.$faza; ?>" rows="2" cols="80" spellcheck="false"><?php echo $training_program_statusi[$key]['komentar_zavrsni']; ?></textarea></td>
				 <td <?php if ($key==$broj_status_mentor-1){echo 'style="display:none;"';}?>>	<select  id="ocjena_mentora" name="<?php echo $item['item_id'].'-'.$usr.'-'.$faza; ?>" class="rcorners1" class="form-control" style="width:190px;outline:none;<?php if($training_program_statusi[$key]['ocjena_mentora']!='' and $training_program_statusi[$key]['ocjena_mentora']!='0' ) echo 'background:#2e9e12;color:white;';?>"  >
     <?php echo _optionObukaOcjenaMentora($training_program_statusi[$key]['ocjena_mentora']); ?>
			   ...
			   <a id="link_potvrda" href="<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>" style="float:right;margin-top:-50px;display:<?php if(!$potpisao_radnik and $potpisao_mentor) {echo '';}else{echo 'none';} ?>" class="table-btn alt1" data-widget="send" data-id="potpisuje_radnik_obuka:<?php echo $_user['user_id'].'-'.$faza;?>" data-text="<?php echo __('Da li ste sigurni da želite potpisati obrazac?'); ?>" data-response="<?php echo __('Obrazac uspješno potpisan'); ?>"><i class="fa fa-arrow-circle-right" style="float:left;color:green;"></i><label style = "font-size:17px;cursor: pointer;">Potpiši obrazac</label></a><br><br><br>
			    <?php } ?>	
				   <?php if ($faza==4){?>
			   <a id="link_potvrda" href="<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>" style="float:right;margin-top:-50px;display:<?php if(!$potpisao_radnik) {echo '';}else{echo 'none';} ?>" class="table-btn alt1" data-widget="send" data-id="potpisuje_radnik_obuka_eval:<?php echo $_user['user_id'].'-'.$faza;?>" data-text="<?php echo __('Da li ste sigurni da želite potpisati obrazac?'); ?>" data-response="<?php echo __('Obrazac uspješno potpisan'); ?>"><i class="fa fa-arrow-circle-right" style="float:left;color:green;"></i><label style = "font-size:17px;cursor: pointer;">Potpiši obrazac</label></a><br><br><br>
			    <?php } ?>	
   ...
   <?php if($potpisao_radnik){
	   ...
	   ?>
   <div class="col-sm-6">
   <br/> <span style="margin-left:0px;background:none;color:black;float:left;"><b><?php echo $evaluator; ?></b> </span><br/>
...
?>
 <script>
    ...
    return regex.test(jQuery(elem)[attr.method](attr.property));
				...
				$.post("<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>", { request: "change-obuka-status", status : this.value, obuka : this.name}, 
    function(returnedData){
		...
		$('textarea:regex(id, .*komentar.*)').on('change', function() {
				...
				$.post("<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>", { request: "change-obuka-komentar", komentar : this.value, obuka : this.name}, 
    function(returnedData){
	...
	$('textarea:regex(id, .*coment.*)').on('change', function() {
				...
				$.post("<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>", { request: "change-obuka-coment", komentar : this.value, obuka : this.name}, 
    function(returnedData){
			...
			$('select:regex(id, .*ocjena_mentora.*)').on('change', function() {
				$.post("<?php echo $url.'/modules/'.$_mod.'/ajax.php'; ?>", { request: "change-obuka-ocjena_mentora", ocjena : this.value, obuka : this.name}, 
    function(returnedData){
		window.location.reload();
});
})
