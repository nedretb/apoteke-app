...
if($_POST['request']=='lang-add'){
  ...
  $get = $db->query("SELECT max([Line No_]) as maximalni FROM  ".$nav_employee_relative."  WHERE [Employee No_]='".$_user['employee_no']."'");
  if($get->rowCount()<0)
  $maximalni = $get->fetchAll();
     
    $data = "INSERT INTO  ".$nav_employee_qualification."  (
    [Employee No_]
...
if($_POST['request']=='remove-kvalifikacija_remove'){
  ...
  $data = "DELETE FROM  ".$nav_employee_qualification."  WHERE [Line No_] = ".$this_id." AND [Employee No_] ='".$_user['employee_no']."'";
...
if($_POST['request']=='travel-request-add'){
  ...
  $query = $db->query("SELECT top 1000 [user_id] FROM  ".$portal_users."  order by user_id");
  ...
  $query = $db->query("SELECT [user_id] FROM  ".$portal_users."  order by user_id offset 1000 rows");
  ...
  $poruka =   _addAllMonths6($_POST['year']);
...
session_write_close();
 ...
 foreach($yearcurr as $value2) {
	...
	$query_department= $db->query("SELECT [B_1_regions],[B_1_regions_description] FROM  ".$portal_departments."  where [code]='".$department."'
   ");  */  
 ...
 echo '<div class="alert alert-success text-center">'.__('Izmjene su uspješno spašene!').'</div>';
  ...
  $go=$db->query("SELECT * FROM  ".$portal_vacation_statistics."  WHERE employee_no='".$empid."'");
   ...
   and year_id=".$getYear);
	...
	$currgo = $db->query("SELECT sum(hour) as sum_hour FROM  ".$portal_hourlyrate_day."  WHERE year_id='".$getYear."'  
   ...
   and year_id=".$getYear);
    ...
    $askedgo0 = $db->query("SELECT count(hour) as count_hour FROM  ".$portal_hourlyrate_day."   where
   ...
   and year_id=".$getYear);
     ...
     $askedgo1 = $db->query("SELECT sum(hour) as sum_hour FROM  ".$portal_hourlyrate_day."   where
		 weekday<>'6' AND weekday<>'7' and
   (
   (day >= ".$day_from." and month_id = ".$month_from." and (".$month_from." <> ".$month_to."))  OR
   (day <= ".$day_to." and month_id = ".$month_to." and (".$month_to." <> ".$month_from.")) OR
  ...
  $currgoPG = $db->query("SELECT sum(hour) as sum_hour FROM  ".$portal_hourlyrate_day."  WHERE year_id='".$getYear."' AND employee_no='".$empid."' 
...
foreach($askedgo0 as $valueasked) {
  $askeddays=$valueasked['count_hour'];
$totalasked0=$askeddays;}

foreach($askedgo1 as $valueasked) {
  $askeddays=$valueasked['sum_hour'];
$totalasked1=$askeddays/8;}
...
foreach($currplo as $valuecurrplo) {
  ...
  $statusi = array('0','0','0','0');
    ...
    $statusi[$x-1]='22';
	  	...
	  	elseif($iskoristenototalpcm+$iskoristenototalupcm+$totalasked<=4){
					 $count_placeni = 0;
					 for ($x = 1; $x <= $day_difference; $x++) {
		if($get_days1[$x-1]['weekday']!='6' and $get_days1[$x-1]['weekday']!='7'){
  ...
  if($pocetak_bolovanja['pocetak_bolovanja']=='')
     $countBolovanje = 0;
	    ...
	    $where_period_prekid = " and (c.Date between '".$pocetak_bolovanja['pocetak_bolovanja']."' and '".$dateFromDB."') and h.user_id=".$_user['user_id'];


  $get_prekid = $db->query("SELECT count(*) as prekid FROM  ".$portal_hourlyrate_day."  h
  join  ".$portal_hourlyrate_year."  y
  on h.year_id = y.id
  join  ".$portal_calendar."  c
  on (c.Year = y.year and c.Day = h.day and c.Month=h.month_id)
  WHERE 
   ((h.status = 5) and (h.KindofDay not in ('SATURDAY','SUNDAY')))".
   $where_period_prekid);
//NK
   $countPrekid = $get_prekid->fetch();

  $where_period_43 = " and (c.Date between '".$pocetak_bolovanja['pocetak_bolovanja']."' and '".$dateFromDB."') and h.user_id=".$_user['user_id'];
...
$max_do = 42;
  ...
  if($entitet!='RS'){
  if(@$get_days2[2]['status']=='44' or ((@$get_days2[1]['KindofDay']=='BHOLIDAY' or @$get_days2[1]['KindofDay']=='SATURDAY' or @$get_days2[1]['KindofDay']=='SUNDAY' or @$get_days2[2]['KindofDay']=='BHOLIDAY' or @$get_days2[2]['KindofDay']=='SATURDAY' or @$get_days2[2]['KindofDay']=='SUNDAY') and @$get_days2[0]['status']=='44' )){
       for ($x = 1; $x <= $day_difference; $x++) 
    $statusi[$x-1]='44';
  }
    elseif(@$get_days2[2]['status']=='43' or ((@$get_days2[1]['KindofDay']=='BHOLIDAY' or @$get_days2[1]['KindofDay']=='SATURDAY' or @$get_days2[1]['KindofDay']=='SUNDAY' or @$get_days2[2]['KindofDay']=='BHOLIDAY' or @$get_days2[2]['KindofDay']=='SATURDAY' or @$get_days2[2]['KindofDay']=='SUNDAY') and @$get_days2[0]['status']=='43')){
         for ($x = 1; $x <= $max_do-$countBolovanje['bolovanje43']; $x++) 
    $statusi[$x-1]='43';
       for ($x = 42-$countBolovanje['bolovanje43']+1; $x <= $day_difference; $x++) {
    $statusi[$x-1]='44';
...
$data = "UPDATE [c0_intranet2_apoteke].[dbo].[bolovanje] SET
  ...
  user_id=?";
	...
	elseif($day_difference>$max_do){
					 for ($x = 1; $x <= $max_do; $x++) 
		$statusi[$x-1]='43';
			 for ($x = 43; $x <= $day_difference; $x++) 
		$statusi[$x-1]='44';
		}}

if($entitet=='RS'){
  if($get_days2[2]['status']=='108' or (($get_days2[1]['KindofDay']=='BHOLIDAY' or $get_days2[1]['KindofDay']=='SATURDAY' or $get_days2[1]['KindofDay']=='SUNDAY' or $get_days2[2]['KindofDay']=='BHOLIDAY' or $get_days2[2]['KindofDay']=='SATURDAY' or $get_days2[2]['KindofDay']=='SUNDAY') and $get_days2[0]['status']=='108' )){
       for ($x = 1; $x <= $day_difference; $x++) 
    $statusi[$x-1]='108';
  }
    elseif($get_days2[2]['status']=='107' or (($get_days2[1]['KindofDay']=='BHOLIDAY' or $get_days2[1]['KindofDay']=='SATURDAY' or $get_days2[1]['KindofDay']=='SUNDAY' or $get_days2[2]['KindofDay']=='BHOLIDAY' or $get_days2[2]['KindofDay']=='SATURDAY' or $get_days2[2]['KindofDay']=='SUNDAY') and $get_days2[0]['status']=='107')){
         for ($x = 1; $x <= $max_do-$countBolovanje['bolovanje43']; $x++) 
    $statusi[$x-1]='107';
       for ($x = 30-$countBolovanje['bolovanje43']+1; $x <= $day_difference; $x++) {
    $statusi[$x-1]='108';
   $data = "UPDATE [c0_intranet2_apoteke].[dbo].[bolovanje] SET
    pocetak_bolovanja = ?
   where 
...
$data = "UPDATE [c0_intranet2_apoteke].[dbo].[bolovanje] SET
  ...
  if($countHol>0 and !in_array($_POST['status'], array(43,44,45,61,62,65,67,68,69,74,75,76,77,78,73,81))){echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete pregaziti praznik!').'</div>';return;}
  if($countOdobreno>0){echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete promjeniti odobrenu registraciju!').'</div>';return;}
 ...
 if($_POST['status']=='84' or $_POST['status']=='67' or $_POST['status']=='106'){
 
 if($_POST['status']=='67')
  $KindofDayString = 'BOLOVANJE';
else
  $KindofDayString = 'BHOLIDAY';


 if($_POST['status']=='67'){
if((@$get_days2[1]['KindofDay']=='BHOLIDAY' or @$get_days2[1]['KindofDay']=='SATURDAY' or @$get_days2[1]['KindofDay']=='SUNDAY') and @$get_days2[0]['status']=='43'){
     $day_from =$day_from-2; 
     
}
if(( @$get_days2[2]['KindofDay']=='BHOLIDAY' or @$get_days2[2]['KindofDay']=='SATURDAY' or @$get_days2[2]['KindofDay']=='SUNDAY') and @$get_days2[0]['status']=='43')
{
     $day_from =$day_from-1;  echo$day_from;
}
 }
 ...
 date_default_timezone_set('Europe/Sarajevo');
   ...
   $number_of_days = cal_days_in_month(CAL_GREGORIAN, $m, 2018);
   if($d>=$number_of_days){
	$d=0;
	$m=$m+1;
	}
 ...
 echo '<div class="alert alert-success text-center">'.__('Izmjene su uspješno spašene!').'</div>';
 //echo 'Ostalo do '.$countBolovanje['bolovanje43'].$entitet;
}
...
foreach($currplo as $valuecurrplo) {
  ...
  on (c.Year = y.year and c.Day = h.day and c.Month=h.month_id)
  WHERE 
   ((h.corr_status = 5) and (h.KindofDay not in ('SATURDAY','SUNDAY')))".
   $where_period_prekid);
//NK
   $countPrekid = $get_prekid->fetch();
...
$max_do = 42;
  ...
  if($entitet!='RS'){
  if(@$get_days2[2]['corr_status']=='44' or ((@$get_days2[1]['KindofDay']=='BHOLIDAY' or @$get_days2[1]['KindofDay']=='SATURDAY' or @$get_days2[1]['KindofDay']=='SUNDAY' or @$get_days2[2]['KindofDay']=='BHOLIDAY' or @$get_days2[2]['KindofDay']=='SATURDAY' or @$get_days2[2]['KindofDay']=='SUNDAY') and @$get_days2[0]['corr_status']=='44' )){
       for ($x = 1; $x <= $day_difference; $x++) 
    $statusi[$x-1]='44';
  }
    elseif(@$get_days2[2]['corr_status']=='43' or ((@$get_days2[1]['KindofDay']=='BHOLIDAY' or @$get_days2[1]['KindofDay']=='SATURDAY' or @$get_days2[1]['KindofDay']=='SUNDAY' or @$get_days2[2]['KindofDay']=='BHOLIDAY' or @$get_days2[2]['KindofDay']=='SATURDAY' or @$get_days2[2]['KindofDay']=='SUNDAY') and @$get_days2[0]['corr_status']=='43')){
         for ($x = 1; $x <= $max_do-$countBolovanje['bolovanje43']; $x++) 
    $statusi[$x-1]='43';
       for ($x = 42-$countBolovanje['bolovanje43']+1; $x <= $day_difference; $x++) {
    $statusi[$x-1]='44';
...
if($entitet=='RS'){
  if(@$get_days2[2]['corr_status']=='108' or ((@$get_days2[1]['KindofDay']=='BHOLIDAY' or @$get_days2[1]['KindofDay']=='SATURDAY' or @$get_days2[1]['KindofDay']=='SUNDAY' or @$get_days2[2]['KindofDay']=='BHOLIDAY' or @$get_days2[2]['KindofDay']=='SATURDAY' or @$get_days2[2]['KindofDay']=='SUNDAY') and @$get_days2[0]['corr_status']=='108' )){
       for ($x = 1; $x <= $day_difference; $x++) 
    $statusi[$x-1]='108';
  }
    elseif(@$get_days2[2]['corr_status']=='107' or ((@$get_days2[1]['KindofDay']=='BHOLIDAY' or @$get_days2[1]['KindofDay']=='SATURDAY' or @$get_days2[1]['KindofDay']=='SUNDAY' or @$get_days2[2]['KindofDay']=='BHOLIDAY' or @$get_days2[2]['KindofDay']=='SATURDAY' or @$get_days2[2]['KindofDay']=='SUNDAY') and @$get_days2[0]['corr_status']=='107')){
         for ($x = 1; $x <= $max_do-$countBolovanje['bolovanje43']; $x++) 
    $statusi[$x-1]='107';
       for ($x = 30-$countBolovanje['bolovanje43']+1; $x <= $day_difference; $x++) {
    $statusi[$x-1]='108';
...
//NK
  ...
  if($countBolovanje['bolovanje43']<42)
   ...
   if(($old_status['KindOfDay']=='BHOLIDAY') and !in_array($_POST['status'], array(43,44,45,61,62,65,67,68,69,74,75,76,77,78,73,81))){echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete pregaziti praznik!').'</div>';return;}
   if($old_status['review_status']=='1' and !(($old_status['KindOfDay']=='BHOLIDAY') and in_array($_POST['status'], array(43,44,45,61,62,65,67,68,69,74,75,76,77,78,73,81))) ){echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete promjeniti odobrenu registraciju!').'</div>';return;}
...
//NK
  ...
  if($countBolovanje['bolovanje43']<42)
   ...
   if(($old_status['KindOfDay']=='BHOLIDAY') and !in_array($_POST['status'], array(43,44,45,61,62,65,67,68,69,74,75,76,77,78,73,81))){echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete pregaziti praznik!').'</div>';return;}
   if($old_status['corr_review_status']=='1' and !(($old_status['KindOfDay']=='BHOLIDAY') and in_array($_POST['status'], array(43,44,45,61,62,65,67,68,69,74,75,76,77,78,73,81))) ){echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete promjeniti odobrenu registraciju!').'</div>';return;}
   if(($totalgoost-1<0)  and $_POST['status']=='18') { echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete unijeti više od dozvoljenih dana GO tekuće godine !').'</div>';return;}
   if (($totalgoostPG-1<0) and $_POST['status']=='19') { echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete unijeti više od dozvoljenih dana GO prethodne godine !').'</div>';return;}
   if(($totalpcmost-1<0) and ($_POST['status']=='21')) { echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete unijeti više od dozvoljenih dana plaćenog vjerskog odsustva!').'</div>';return;}
   if(($totalupcmost-1<0) and ($_POST['status']=='22')) { echo '<div class="alert alert-danger"><b>'.__('Greška!').'</b><br/>'.__('Ne možete unijeti više od dozvoljenih dana neplaćenog vjerskog odsustva!').'</div>';return;}
...
echo $host.'/CORE/public/Odsustva_'.$_user['username'].'('.$pocetni_datum.' - '.$krajnji_datum.').pdf';
  ...
  if($_POST['request']=='profile-edit'){
   ...
   $get = $db->query("SELECT * FROM  ".$nav_employee."  WHERE [No_]='".$this_id."'");
  if($get->rowCount()<0)
...
// hm 01
     if(isset($_FILES['media_file'])){
       if(is_uploaded_file($_FILES['media_file']['tmp_name'])){
		   $p_photo_x = base64_encode(file_get_contents($_FILES['media_file']['tmp_name']));
		   ...
		   if($_FILES["media_file"]["type"] == "image/jpeg" || $_FILES["media_file"]["type"] == "image/jpg"){
				...
				if(move_uploaded_file($_FILES['media_file']['tmp_name'], 'D:/Temp/uploads/raiff/' . $this_id . ".jpeg" )){
...
$count_srodnici_banka = 0;
 ...
 $get = $db->query("SELECT * FROM  ".$nav_employee_relative."  WHERE [Employee No_]='".$this_id."' AND [Relative Code] = 'OTAC'");
  if($get->rowCount()<0)
  $row_otac = $get->fetchAll();

$get = $db->query("SELECT * FROM  ".$nav_employee_relative."  WHERE [Employee No_]='".$this_id."' AND [Relative Code] = 'MAJKA'");
  if($get->rowCount()<0)
  $row_majka = $get->fetchAll();

$get = $db->query("SELECT * FROM  ".$nav_employee_relative."  WHERE [Employee No_]='".$this_id."' AND [Relative Code] = N'SUPRUŽNIK'");
  if($get->rowCount()<0)
...
if(!isset($_POST['phone_emergency_person'])){
  $_POST['phone_emergency_person'] = "";
}
if(!isset($_POST['Phone_No'])){
  $_POST['Phone_No'] = "";
}
if(!isset($_POST['phone_mob'])){
  $_POST['phone_mob'] = "";
}


// hm 02 kod od 6509 do 6542
if(((strpos($_POST['Phone_No'], ' ') <3 && $_POST['Phone_No'] != "") or (strpos($_POST['Phone_No'], ' ') > 3 && $_POST['Phone_No'] != "")) or ((strpos($_POST['phone_mob'], ' ') <3 && $_POST['phone_mob'] != "") or (strpos($_POST['phone_mob'], ' ') > 3 && $_POST['phone_mob'] != "")) or ((strpos($_POST['phone_emergency_person'], ' ') <3 && $_POST['phone_emergency_person'] != "" ) or (strpos($_POST['phone_emergency_person'], ' ') > 3 && $_POST['phone_emergency_person'] != "")) ){
...
echo '{"jsonrpc" : "2.0", "status" : "ok", "msg" : "<div class=\"alert alert-danger text-center\">'.__('Format telefona nije ispravan').'</div>"}';
  ...
  $this_id = $_POST['request_id'];
    ...
    $data = "UPDATE  ".$nav_employee_relative."  SET
    [First Name] = ?,
    [Last Name] = ?,
    [Birth Date] = ?
    WHERE [Employee No_] = ?
    AND [Relative Code] = 'OTAC'";
     ...
     $this_id
        ...
        $rodjenje_otac = date("Y/m/d", strtotime(str_replace("/",".",$_POST['rodjenje_otac'])));
          ...
          $get = $db->query("SELECT max([Line No_]) as maximalni FROM  ".$nav_employee_relative."  WHERE [Employee No_]='".$this_id."'");
  if($get->rowCount()<0)
  $maximalni = $get->fetchAll();

    $data = "INSERT INTO  ".$nav_employee_relative."  ([Employee No_],[Line No_],
...
VALUES (?, ?, ?, ?, ?,?, ?, ?, ?, ?,?, ?, ?, ?, ?,?, ?, ?, ?, ?)";
   ...
   $ukupni_jezici  = array();
       ...
       foreach($ukupni_jezici as $key=>$nivo){
        $data2 = "UPDATE  ".$nav_employee_qualification."  SET [Language Level] = ".$nivo['nivo_jezik'].", [Language Name]= '".$nivo['jezik']."' WHERE [Line No_] = ".$nivo['line_no']." AND [Employee No_] ='".$this_id."'";
      ...
      $res2 = $db->prepare($data2);
       ...
       foreach($ukupni_certifikati as $key=>$certifikat){
        $data2 = "UPDATE  ".$nav_employee_qualification."  SET [Description] = '".$certifikat['opis']."', [Institution_Company]= '".$certifikat['kompanija']."' WHERE [Line No_] = ".$certifikat['line_no']." AND [Employee No_] ='".$this_id."'";
  ...
  if(@$_POST['zamjenik']!=''){
    ...
    parent2 = ".$_POST['zamjenik']." where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parent='"._user($this_id)['employee_no']."'))";
      ...
      $res->execute(
       ...
       $data = "UPDATE  ".$portal_users."  SET
    parentMBO2d = ".$_POST['zamjenik']." where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO2='"._user($this_id)['employee_no']."'))";
     ...
     $data = "UPDATE  ".$portal_users."  SET
    parentMBO3d = ".$_POST['zamjenik']." where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO3='"._user($this_id)['employee_no']."'))";
      ...
      $res->execute(
       ...
       $data = "UPDATE  ".$portal_users."  SET
    parentMBO4d = ".$_POST['zamjenik']." where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO4='"._user($this_id)['employee_no']."'))";
      ...
      $data = "UPDATE  ".$portal_users."  SET
    parentMBO5d = ".$_POST['zamjenik']." where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO5='"._user($this_id)['employee_no']."'))";
    ...
    parent2 = 0 where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parent='"._user($this_id)['employee_no']."'))";
     ...
     $data = "UPDATE  ".$portal_users."  SET
    parentMBO2d = 0 where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO2='"._user($this_id)['employee_no']."'))";
      ...
      $data = "UPDATE  ".$portal_users."  SET
    parentMBO3d = 0 where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO3='"._user($this_id)['employee_no']."'))";
     ...
     $data = "UPDATE  ".$portal_users."  SET
    parentMBO4d = 0 where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO4='"._user($this_id)['employee_no']."'))";
     ...
     $data = "UPDATE  ".$portal_users."  SET
    parentMBO5d = 0 where employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE (parentMBO5='"._user($this_id)['employee_no']."'))";
...
echo '{"jsonrpc" : "2.0", "status" : "ok", "msg" : "<div class=\"alert alert-danger text-center\">'.__('Format telefona nije ispravan').'</div>"}';
  ...
  echo $srodstvo;
  $code_srodstva= $db->query("SELECT [Code] as sifras FROM  ".$nav_relative." WHERE [Code]='".$_POST['srodstvo']."'");
...
<?php }
  ...
  if($_POST['request']=='business-trip-request-add'){
    ...
    // $parent2_query  = $db->query("SELECT * FROM  ".$portal_users."  WHERE employee_no='".$_user['parentMBO2']."'");
...
$_user = _user(_decrypt($_SESSION['SESSION_USER'])); 			
		 ...
		 $employee_query = " and employee_no =".$_user['employee_no'];
			...
			$get_accepted = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
		join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
		WHERE 
   (
   weekday<>'6' AND weekday<>'7' and status<>5 ".$employee_query.$odobreno_query.")
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result2 = $get_accepted->fetch();
      $total_accepted=$result2[0];
	  
	       $get_pending = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
		   join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
		   WHERE 
   (
   weekday<>'6' AND weekday<>'7' and status<>5 ".$employee_query.$neodobreno_query.")
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result2 = $get_pending->fetch();
      $total_pending=$result2[0];
	  
	         $get_rejected = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
			 join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
			 WHERE 
   (
   weekday<>'6' AND weekday<>'7' and status=5 ".$employee_query.$neodobreno_query.")
   and year = ".$godina."
   and timest_edit is not null");
	   $result2 = $get_rejected->fetch();
      $total_rejected=$result2[0];
	  
	    
		$get_accepted_cancel = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
		join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
		WHERE 
   (
   weekday<>'6' AND weekday<>'7'  ".$employee_query.") and change_req=2 and status=5
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result3 = $get_accepted_cancel->fetch();
      $total_accepted_cancel=$result3[0];
	  
	  $get_rejected_cancel = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
		join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
		WHERE 
   (
   weekday<>'6' AND weekday<>'7'  ".$employee_query.") and change_req='0'
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result3 = $get_rejected_cancel->fetch();
      $total_rejected_cancel=$result3[0];
		
		echo $total_accepted.','.$total_pending.','.$total_rejected.','.$total_accepted_cancel.','.$total_rejected_cancel;
...
$_user = _user(_decrypt($_SESSION['SESSION_USER'])); 			
	     ...
	     $odobreno_query=" and review_status = 1";
		  $corr_odobreno_query=" and review_status = 1";
		 $employee_query = " and employee_no =".$_user['employee_no'];
...
//za admina
	...
	/*   $get_absences_view = $db->query("SELECT absence_view FROM  ".$portal_absence_misc."  WHERE employee_no = ".$_user['employee_no']."");
	   $result = $get_absences_view->fetch();
      $absences_view=$result['absence_view']; */
	  
	  $role_query = " and employee_no in (SELECT employee_no FROM  ".$portal_users."  WHERE ".$_user['employee_no']." in (admin1,admin2,admin3,admin4,admin5,admin6,admin7,admin8))";

	  $get_absences = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
	  join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
	  WHERE 
   (
   status<>5 ".$role_query.$neodobreno_query.")
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result = $get_absences->fetch();
      $total_absences=$result[0];
	  
	    $get_absences_corr = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
		 join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
		WHERE 
   (
   corr_status<>5 ".$role_query.$corr_neodobreno_query.")
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result = $get_absences_corr->fetch();
      $total_absences_corr=$result[0];
	  
	    $get_chane_req = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
		 join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
		WHERE 
   (
   weekday<>'6' AND weekday<>'7' and status<>5 and change_req=1 ".$role_query.$odobreno_query.")
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result1 = $get_chane_req->fetch();
      $total_change_req=$result1[0];
	  
	    $get_chane_req_corr = $db->query("SELECT count(*) FROM  ".$portal_hourlyrate_day."  
		join  ".$portal_hourlyrate_year."  on
	 ".$portal_hourlyrate_day." .year_id =  ".$portal_hourlyrate_year." .id
		WHERE 
   (
   weekday<>'6' AND weekday<>'7' and corr_status<>5 and corr_change_req=1 ".$role_query.$corr_odobreno_query.")
   and year = ".$godina."
   --and timest_edit >'".'1'."'");
	   $result1 = $get_chane_req_corr->fetch();
      $total_change_req_corr=$result1[0];
