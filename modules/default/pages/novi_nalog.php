...
$admin = $db->query("SELECT count(user_id) as br FROM  " . $portal_users . "  where sl_put_admin=1 and user_id=" . $_user['user_id']);
$admin = $admin->fetch();
...
if ($_POST) {
    ...
    if (empty($_POST['sl_put_id'])) {
        ...
        if (isset($_GET['get_year'])) {
            $check_status = $db->query("SELECT count(*) as c FROM  " . $portal_hourlyrate_day . "  as h
inner join  " . $portal_hourlyrate_year . "  as y on y.user_id = h.user_id
...
employee_no = " . $_POST['employee_no'] . " and (status not in (83, 5) and corr_status not in (83, 5) or change_req = 1)");
            ...
            $check_status = $db->query("SELECT count(*) as c FROM  " . $portal_hourlyrate_day . "  where Date >='" . $begin_date . "' and  Date <='" . $end_date . "' and 
employee_no = " . $_POST['employee_no'] . " and (status not in (83, 5) and corr_status not in (83, 5) or change_req = 1)");
            $check_status = $check_status->fetch();
        }
        //status
        ...
        if ($time_poc > strtotime($begin_date)) {
            $check_editable = $db->query("SELECT count(*) as c FROM  " . $portal_hourlyrate_day . "  where Date >='" . $begin_date . "' and  Date <='" . $end_date . "' and 
...
employee_no = " . $_POST['employee_no'] . " and editable ='N' ");
        ...
        //editing sl put
        $get_sl_put = $db->query("SELECT id,sp.status,sp.request_id,sp.pocetak_datum,sp.kraj_datum,sp.kraj_datum2 FROM  " . $portal_sl_put . "  as sp where sp.id='" . $_POST['sl_put_id'] . "'");
        ...
        //check status
        $check_status = $db->query("SELECT count(*) as c FROM  " . $portal_hourlyrate_day . "  
  ...
  employee_no = " . $_POST['employee_no'] . " and (status not in (83, 5) and corr_status not in (83, 5) or change_req = 1) and id not in (
   SELECT id FROM  " . $portal_hourlyrate_day . "  
   ...
   and employee_no = " . $_POST['employee_no'] . ")");
        ...
        $check_editable['c'] = 0;
        if ($time_poc > strtotime($begin_date)) {
            $check_editable = $db->query("SELECT count(*) as c FROM  " . $portal_hourlyrate_day . "  
                  where Date >='" . $begin_date . "' and  Date <='" . $end_date . "' and 
    ...
    if (strtotime($get_sl_put['pocetak_datum']) != strtotime($_POST['pocetak_datum']) or strtotime($get_sl_put['kraj_datum']) != strtotime($_POST['kraj_datum']) or strtotime($get_sl_put['kraj_datum2']) != strtotime($_POST['kraj_datum2']) or empty($_POST['sl_put_id']) or $get_sl_put['svrha'] != $_POST['svrha']) {
        ...
        if ($_POST['svrha'] != 'UPOTREBA VLASTITOG AUTOMOBILA U SLUŽBENE SVRHE') {
          ...
          timest_edit = '" . date('Y-m-d h:i:s') . "',
            ...
            $novi_put = $db->query("UPDATE  " . $portal_hourlyrate_day . "  set 
          status = 
          case 
    ...
    if (true) {
        try {
            ...
            if (empty($_GET['sl_put_id'])) {
                $data = $db->query("INSERT INTO  " . $portal_sl_put . "  ([request_id],[status],[svrha],[pocetak_datum],[pocetak_vrijeme],[kraj_datum],[kraj_vrijeme],[polazna_drzava],[grad_polaska],[odredisna_drzava],[odredisni_grad],[odredisna_drzava2],[odredisni_grad2],[odredisna_drzava3],[odredisni_grad3],[razlog_putovanja],[napomena],[iznos_akontacije],[valuta],[datum_akontacije],[primanje_sredstva],[akontacija_napomena],[vrsta_transporta],[transport_pocetak_datum],[transport_pocetak_vrijeme],[transport_kraj_datum],[transport_kraj_vrijeme],[transport_polazna_drzava],[transport_grad_polaska],[transport_odredisna_drzava],[transport_odredisni_grad],[transport_napomena],[vrsta_smjestaja],[smjestaj_pocetak_datum],[smjestaj_pocetak_vrijeme],[smjestaj_kraj_datum],[smjestaj_kraj_vrijeme],[smjestaj_drzava],[smjestaj_grad],[smjestaj_adresa],[osiguranje],[osiguranje_pocetak_datum],[osiguranje_pocetak_vrijeme],[osiguranje_kraj_datum],[osiguranje_kraj_vrijeme],[dokument_broj],[viza],[osiguranje_napomena],[lock],[status_hr],[created_at],kraj_datum2,kraj_vrijeme2,kategorija_hotela,placeno_biznis_karticom,za_isplatu_povrat,smjestaj_napomena,employee)
    VALUES (" . "'" .
                    $_POST['request_id'] . "','" .
                    $_POST['status'] . "','" .
                    $_POST['svrha'] . "','" .
    ...
    '" . $_POST['employee_no'] . "'
                ...
                $data_ostalo = $db->query("INSERT INTO  " . $portal_sl_put_ostali_info . "  (
        [request_id],
        [trosak1],
        [trosak2],
        [kolicina1],
        ...
        " . $db->lastInsertId() . "
                ...
                $data_before_updateq = $db->query("select * from  " . $portal_sl_put . "  as s join  " . $portal_sl_put_ostali_info . "  as i on i.sl_put_id_fk = s.id where id =" . $_POST['sl_put_id']);
                $data_before_update = $data_before_updateq->fetch(PDO::FETCH_ASSOC);
                $data_ostalo = $db->query("UPDATE  " . $portal_sl_put_ostali_info . "  SET
...
//nova notifikacija finansije
    ...
    SELECT *  FROM  " . $portal_hourlyrate_day . "  as table1
    INNER JOIN  " . $portal_sl_put . "  as table2
    ON table1.id = table2.request_id 
    inner join  " . $portal_users . "  as table3
    ...
    ");
                    ...
                    $parentq = $db->query("SELECT email_company, fname, lname from  " . $portal_users . "  WHERE employee_no = " . $podaci_mail['parentMBO2'] . " ");
...
//
  ...
  inner join  " . $portal_users . "  as table3
  ON table1.user_id = table3.user_id
  left join  " . $portal_countries . "  as c on c.country_id = table2.odredisna_drzava
...
//kraj notifikacije o izmjeni
        ...
        if (empty($_GET['sl_put_id'])) {
            ...
            if (empty($_GET['sl_put_id'])) {
                $new_slput = $db->query("SELECT top(1) id from  " . $portal_sl_put . "  order by id desc ");
    ...
    ");
      ...
      ";
        ...
        if (isset($_POST['obrada'])) {
            ...
            $last_log = $db->query("Select top(1) * from  " . $portal_sl_put_logs . "  where sl_put_request_id = " . $_POST['sl_put_id'] . " order by vrijeme desc ");
            $last_log = $last_log->fetch();

            if ($last_log['operation'] == 'odbijanje') {


                $provjeraq = $db->query("SELECT count(*) as aa FROM  " . $portal_sl_put . "  where status_hr = '1' and id =" . $_POST['sl_put_id']);
                foreach ($provjeraq as $one) {
                    $provjera = $one['aa'];
                }

                if ($one['aa'] == 0) {
                    $odobri_req = $db->query("UPDATE  " . $portal_sl_put . "  set status_hr = 1 where id =" . $_POST['sl_put_id']);
                    $odobri_req->execute();
                    //logs
    ...
    VALUES (" . $_POST['sl_put_id'] . ", 'odobravanje', $user, $ts)");
                ...
                $odobri_req2 = $db->query("UPDATE  " . $portal_sl_put . "  set na_obradi = 1 where id =" . $_POST['sl_put_id']);
                $odobri_req2->execute();
    ...
    " . time() . "
        ...
        header("Location: /apoteke-app/?m=business_trip&p=all&pg=1");
        exit();
    }
}
...
$cijena_goriva_postotak = $db->query("SELECT * from  " . $portal_sifrarnici . "  where active = 1 and name ='cijena_goriva_postotak'");

$sifrarnik_drzave = $db->query("SELECT * from  " . $portal_countries . "  ORDER BY name COLLATE Latin1_General_CS_AS_KS_WS ASC ");
...
if (!empty($_GET['sl_put_id'])) {
    $putq = $db->query("SELECT * FROM  " . $portal_sl_put . "  where id =" . $_GET['sl_put_id']);
    $put = $putq->fetch();
} else $put = null;

if (isset($_GET['sl_put_id'])) {
    $putb = $db->query("SELECT * FROM  " . $portal_sl_put_ostali_info . "  where sl_put_id_fk ='" . $_GET['sl_put_id'] . "'");
    $putt = $putb->fetch();
} else $putt = null;

// postavljanje datuma na selektovani
if (isset($_GET['get_year'])) {
    $check_status = $db->query("SELECT * FROM  " . $portal_users . "  as u 
left join  " . $portal_hourlyrate_year . "  as y on y.user_id= u.user_id
left join  " . $portal_hourlyrate_day . "  as d on d.user_id= u.user_id
where Date = '" . date("Y-m-d", strtotime($_GET['dateOD'])) . "' and y.id=  " . $_GET['get_year']);
    $hd_data = $check_status->fetch();

} else {
    $hd_data = $db->query("SELECT * from  " . $portal_hourlyrate_day . "  where id = " . $_GET['id']);
...
while ($tip == 'BHOLIDAY' or in_array($dan, [6, 7])) {
    $id--;
    $dan_data = $db->query("SELECT * from  " . $portal_hourlyrate_day . "  where id = $id ");
...
<section class="full">
    ...
    if (isset($_SESSION['datumi_err'])) {
        unset($_SESSION['datumi_err']); ?>
    ...
    <?php }
    if (isset($_SESSION['zakljucano_err'])) {
        unset($_SESSION['zakljucano_err']); ?>
        <div class="container row alert alert-danger" role="alert" style='margin-top:10px;'>
    ...
    <div class='container row box' style='text-align:center;padding:10px;margin-top:15px;height:50px;'>
        ...
        <div style="display:inline;float:right;"><a href="/apoteke-app/?m=business_trip&p=all&pg=1"
                                                    style="background-color:#f5f5f5" class="btn box-head-btn">X</a>
        </div>
    </div>
        ...
        <div class="container row box">
            ...
            <div class='content' id='c1'>
                <div class="col-sm-12">
                    <div class="col-sm-12">
                        <label>Svrha</label>
                        <select name="svrha" id="svrha" onchange="promjenjena_svrha(this);" class="form-control"
                                required oninvalid="this.setCustomValidity('Molimo vas da odeberete opciju')"
                            ...
                            if ($put['svrha']) {
                                if ($put['svrha'] == 'UPOTREBA VLASTITOG AUTOMOBILA U SLUŽBENE SVRHE') {
                                    ?>
                                    <option selected value="UPOTREBA VLASTITOG AUTOMOBILA U SLUŽBENE SVRHE">UPOTREBA
                                        VLASTITOG AUTOMOBILA U SLUŽBENE SVRHE
                                    </option>
                                    <?php
                                } else {
                                    foreach ($sifrarnik_svrha as $one) {
                                        if ($one['naziv_instance'] != 'UPOTREBA VLASTITOG AUTOMOBILA U SLUŽBENE SVRHE') {
                                            ...
                                            <option <?php echo $selected; ?>
                                                    value="<?php echo $one['naziv_instance'] ?>"><?php echo $one['naziv_instance'] ?></option>
                                            <?php
                                        }
                ...
                <div class="col-sm-12">
                    ...
                    <div class="col-sm-3">
                        ...
                        <input type="text" name="grad_polaska" id='grad_polaska'
                               value="<?php if (isset($put)) echo $put['grad_polaska']; else echo ''; ?>"
                               placeholder='Unesite puni naziv' class="form-control">
                    </div>
                    <div class="col-sm-3">
                        <label>Početak datum:</label>
                        <input type="text" id="pocetak_datum" name="pocetak_datum" value="<?php echo $pocetak_sl_put ?>"
                               readonly class="form-control"
                               onchange="promjeni_pocetak(this.value); change_datum_akontacije();">
                    </div>

                    <div class="col-sm-3">
                        <label>Početak vrijeme:</label>
                        <input class="time-input form-control timepicker" type="text" name="pocetak_vrijeme"
                               value="<?php if (isset($put)) echo $put['pocetak_vrijeme']; else echo ''; ?>"
                               class="form-control">
                    </div>
                </div>
                ...
                <div class="col-sm-12">
                    ...
                    <div class="col-sm-3">
                        ...
                        <input type="text" name="odredisni_grad" id='odredisni_grad'
                               value="<?php if (isset($put)) echo $put['odredisni_grad']; else echo ''; ?>"
                               placeholder='Unesite puni naziv' class="form-control">
                    </div>
                    <div class="col-sm-3">
                        <label>Krajnji datum u odredišnoj državi:</label>
                        <input type="text" name="kraj_datum" id="kraj_datum" value="<?php echo $kraj_sl_put; ?>"
                               class="form-control" onchange="promjeni_kraj(this.value);">
                    </div>
                    <div class="col-sm-3">
                        <label>Krajnje vrijeme u odredišnoj državi:</label>
                        <input class="time-input form-control  timepicker" type="text" name="kraj_vrijeme"
                               id='kraj_vrijeme'
                               value="<?php if (isset($put)) echo $put['kraj_vrijeme']; else echo ''; ?>"
                               class="form-control">
                    </div>
                </div>
                ...
                <div class="col-sm-12">
                    ...
                    <div class="col-sm-3">
                        ...
                        <input type="text" name="odredisni_grad2" id='odredisni_grad2'
                               value="<?php if (isset($put)) echo $put['odredisni_grad2']; else echo ''; ?>"
                               placeholder='Unesite puni naziv' class="form-control">
                    </div>
                    <div class="col-sm-3">
                        <label>Krajnji datum u odredišnoj državi 2:</label>
                        <input type="text" name="kraj_datum2" id='kraj_datum2' value="<?php echo $kraj_sl_put2; ?>"
                               onchange="promjeni_kraj(this.value);" class="form-control">
                    </div>
                    <div class="col-sm-3">
                        <label>Krajnje vrijeme u odredišnoj državi 2:</label>
                        <input class=" form-control " readonly type="text" name="kraj_vrijeme2" id='kraj_vrijeme2'
                               value="<?php if (isset($put)) echo $put['kraj_vrijeme2']; else echo ''; ?>"
                               class="form-control">
                    </div>
                </div>
                <!-- <div class="col-sm-12" >
                ...
                <div class="col-sm-12">
                    <div class="col-sm-6">
                        ...
                        <textarea name="razlog_putovanja" id='razlog_putovanja'
                                  value="<?php if (isset($put)) echo $put['razlog_putovanja']; else echo ''; ?>"
                                  class="form-control"></textarea>
                    </div>
                    <div class="col-sm-6">
                        <label>Napomena:</label>
                        <textarea name="napomena" id='napomena'
                                  value="<?php if (isset($put)) echo $put['napomena']; else echo ''; ?>"
                                  class="form-control"></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class='content' id='c2'>
                ...
                <div class="col-sm-12">
                    <div class="col-sm-6">
                        ...
                        <input step="any" type="number" name="iznos_akontacije" id='iznos_akontacije'
                               value="<?php if (isset($put)) echo $put['iznos_akontacije']; else echo ''; ?>"
...
<!--                </div>-->
                ...
                <div class="col-sm-12">
                    <div class="col-sm-12">
                        <label>Napomena:</label>
                        <textarea name="akontacija_napomena" id='akontacija_napomena'
                                  value="<?php if (isset($put)) echo $put['akontacija_napomena']; else echo ''; ?>"
                                  class="form-control"></textarea>
                    </div>
                </div>
            </div>
        ...
        <div class="container row box">
            ...
            <div class='content' id='c3'>
                <div class="col-sm-12">
                    <div class="col-sm-12">
                        <label>Prevozno sredstvo u odlasku:</label>
                        <select name="vrsta_transporta" class="form-control">
                            <option value=" ">Odaberi...</option>
                            <?php
                            foreach ($sifrarnik_vrsta_transporta as $one) {
                                ?>
                                <option <?= ($one['naziv_instance'] == $put['vrsta_transporta']) ? 'selected' : '' ?> value="<?= $one['naziv_instance']; ?>"><?= $one['naziv_instance']; ?></option>
                                <?php
                            }
                            ?>
                        </select>
                    ...
                    <div class="col-sm-12">
                        <label>Prevozno sredstvo u odlasku:</label>
                        <select name="transport_napomena" class="form-control">
                            <option value="">Odaberite</option>
                            <?php
                            $sifarnik = $db->query("SELECT * from  " . $portal_sifrarnici . "  where active = 1 and name ='vrsta_transporta'");
                            foreach ($sifarnik as $one) {
                                ?>
                                <option <?= ($one['naziv_instance'] == $put['transport_napomena']) ? 'selected' : '' ?> value="<?= $one['naziv_instance']; ?>"><?= $one['naziv_instance']; ?></option>
                                <?php
                            }
                            ?>
                        </select>
                ...
                <div class="col-sm-12">
                    <div class="col-sm-12">
                        <div class="col-sm-3">
                            ...
                            <input type="text" name="trosak1" id='trosak1'
                                   value="<?php if (isset($putt)) echo $putt['trosak1']; else echo ''; ?>"
                                   placeholder='Unesite naziv troška prevoza' class="form-control">

                        </div>
                        <div class="col-sm-3">
                            <label>Količina:</label>
                            <input step="any" type="number" min="0" name="kolicina1" id='kolicina1'
                                   onchange="calculate_total_nocenja2();"
                                   value="<?php if (isset($putt)) echo $putt['kolicina1']; else echo ''; ?>"
                                   placeholder='Unesite količinu' class="form-control">

                        </div>
                        <div class="col-sm-3">
                            <label>Iznos KM:</label>
                            <input step="any" type="number" min="0" name="iznos1" id='iznos1'
                                   onchange="calculate_total_nocenja2();"
                                   value="<?php if (isset($putt)) echo $putt['iznos1']; else echo ''; ?>"
                                   placeholder='Unesite iznos' class="form-control">
                    ...
                    <div class="col-sm-12" id="prevoz_red2"
                         style="<?php if (!$putt['trosak2']) echo 'display:none;'; ?>">
                        <div class="col-sm-3">

                            <input type="text" name="trosak2" id='trosak2'
                                   value="<?php if (isset($putt)) echo $putt['trosak2']; else echo ''; ?>"
                                   placeholder='Unesite naziv troška prevoza' class="form-control">

                        </div>
                        <div class="col-sm-3">

                            <input step="any" type="number" min="0" name="kolicina2" id='kolicina2'
                                   onchange="calculate_total_nocenja2();"
                                   value="<?php if (isset($putt)) echo $putt['kolicina2']; else echo ''; ?>"
                                   placeholder='Unesite količinu' class="form-control">

                        </div>
                        <div class="col-sm-3">

                            <input step="any" type="number" min="0" name="iznos2" id='iznos2'
                                   onchange="calculate_total_nocenja2();"
                                   value="<?php if (isset($putt)) echo $putt['iznos2']; else echo ''; ?>"
                                   placeholder='Unesite iznos' class="form-control">
                    ...
                    <div class="col-sm-12">
                        <div class="btn-sm btn-info mybtn" style="<?php if ($putt['trosak2']) echo 'display:none;'; ?>"
                             id="add_prevoz"
                             onclick="$('#prevoz_red2').show(); $('#delete_prevoz').show(); $('#add_prevoz').hide();">
                            Dodaj red +
                        </div>

                        <div class="btn-sm btn-danger mybtn"
                             style="<?php if ($putt['trosak2']) echo 'display:block;'; else echo 'display:none;' ?>"
                             id="delete_prevoz"
                             onclick="$('#prevoz_red2').hide();$('#delete_prevoz').hide();$('#add_prevoz').show();$('#trosak2').val('');$('#kolicina2').val('');$('#iznos2').val('');calculate_total_nocenja2();">
                            Izbriši red
                        </div>
                    ...
                    <div class="col-sm-12">
                        <div class="col-sm-12"
                             style="border-top: 1px solid #d8cdcd;padding-top: 8px;font-size: 15px;padding-left: 0px;">
                            <p>U slučaju da ste koristili vlastiti automobil molimo Vas popunite pređene kilometre.</p>
                            <div class="col-sm-4">

                                <input step="any" type="number" min="0" name="kol_gorivo" id='kol_gorivo'
                                       onchange="calculate_gorivo()"
                                       value="<?php if (isset($putt)) echo $putt['kol_gorivo']; else echo ''; ?>"
                                       placeholder='Unesite pređene kilometre (km)' class="form-control">

                            </div>
                            <div class="col-sm-4">

                                <select name="iznos_gorivo" class="form-control" id='iznos_gorivo'
                                        onchange="calculate_gorivo()">
                                    <?php
                                    foreach ($cijena_goriva_postotak as $one) {
                                        ...
                                        ?>
                                        <option data-km="<?php echo $matches[1][0]; ?>"
                                                data-postotak="<?php echo $matches[3][0]; ?>" <?php echo $selected; ?>
                                                value="<?php echo $one['naziv_instance']; ?>"><?php echo $one['naziv_instance']; ?></option>
                                        <?php
                                    }
                                    ?>
                ...
                <div class="col-sm-12">
                    <div class="col-sm-12">
                        <div class="col-sm-3">
                            ...
                            <input type="text" name="izdaci_naziv1" id='izdaci_naziv1'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_naziv1']; else echo ''; ?>"
                                   placeholder='Unesite naziv' class="form-control">

                        </div>
                        <div class="col-sm-3">
                            <label>Broj noćenja:</label>
                            <input step="any" type="number" min="0" name="izdaci_kol1" id='izdaci_kol1'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_kol1']; else echo ''; ?>"
                                   placeholder='Unesite količinu' class="form-control">

                        </div>
                        <div class="col-sm-3">
                            <label>Iznos KM po noćenju:</label>
                            <input step="any" type="number" min="0" name="izdaci_iznos1" id='izdaci_iznos1'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_iznos1']; else echo ''; ?>"
                                   placeholder='Unesite iznos' class="form-control">
                    ...
                    <div class="col-sm-12" id="nocenja_red2"
                         style="<?php if (!$putt['izdaci_naziv2']) echo 'display:none;'; ?>">
                        <div class="col-sm-3">

                            <input type="text" name="izdaci_naziv2" id='izdaci_naziv2'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_naziv2']; else echo ''; ?>"
                                   placeholder='Unesite naziv' class="form-control">

                        </div>
                        <div class="col-sm-3">

                            <input step="any" type="number" min="0" name="izdaci_kol2" id='izdaci_kol2'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_kol2']; else echo ''; ?>"
                                   placeholder='Unesite količinu' class="form-control">

                        </div>
                        <div class="col-sm-3">

                            <input step="any" type="number" min="0" name="izdaci_iznos2" id='izdaci_iznos2'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_iznos2']; else echo ''; ?>"
                                   placeholder='Unesite iznos' class="form-control">
                    ...
                    <div class="col-sm-12" id="nocenja_red3"
                         style="<?php if (!$putt['izdaci_naziv3']) echo 'display:none;'; ?>">
                        <div class="col-sm-3">

                            <input type="text" name="izdaci_naziv3" id='izdaci_naziv3'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_naziv3']; else echo ''; ?>"
                                   placeholder='Unesite naziv' class="form-control">

                        </div>
                        <div class="col-sm-3">

                            <input step="any" type="number" min="0" name="izdaci_kol3" id='izdaci_kol3'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_kol3']; else echo ''; ?>"
                                   placeholder='Unesite količinu' class="form-control">

                        </div>
                        <div class="col-sm-3">

                            <input step="any" type="number" min="0" name="izdaci_iznos3" id='izdaci_iznos3'
                                   value="<?php if (isset($putt)) echo $putt['izdaci_iznos3']; else echo ''; ?>"
                                   placeholder='Unesite iznos' class="form-control">
                    ...
                    <div class="col-sm-12">
                        <div class="btn-sm btn-info mybtn" id="prvo_dugme"
                             onclick="$('#nocenja_red2').show(); $('#drugo_dugme').show(); $('#prvo_dugme').hide();  $('#delete_secound').show();">
                            Dodaj red +
                        </div>
                        <div class="btn-sm btn-info mybtn" style="display:none;" id="drugo_dugme"
                             onclick="$('#nocenja_red3').show(); $('#drugo_dugme').hide(); $('#delete_third').show();$('#delete_secound').hide();">
                            Dodaj red +
                        </div>

                        <div class="btn-sm btn-danger mybtn" style="display:none;" id="delete_secound"
                             onclick="$('#nocenja_red2').hide();$('#delete_secound').hide();$('#prvo_dugme').show();$('#drugo_dugme').hide();$('#izdaci_naziv2').val('');$('#izdaci_kol2').val('');$('#izdaci_iznos2').val('');calculate_total_nocenja();">
                            Izbriši red
                        </div>

                        <div class="btn-sm btn-danger mybtn" style="display:none;" id="delete_third"
                             onclick="$('#nocenja_red3').hide();$('#delete_third').hide();$('#drugo_dugme').show();$('#delete_secound').show(); $('#izdaci_naziv3').val('');$('#izdaci_kol3').val('');$('#izdaci_iznos3').val('');calculate_total_nocenja();">
                            Izbriši red
                        </div>
                    </div>
                </div>
                ...
                <div class="col-sm-12" id="TextBoxesGroup">
                    ...
                    <div class="col-sm-12 ultimate_class" id="TextBoxDiv">
                        <div class="col-sm-3">
                            ...
                            <input type="text" name="ost_trosak1" id='ost_trosak1'
                                   value="<?php if (isset($putt)) echo $putt['ost_trosak1']; else echo ''; ?>"
                                   placeholder='Unesite naziv troška' class="form-control">

                        </div>
                        <div class="col-sm-3">
                            <label>Količina:</label>
                            <input step="any" type="number" min="0" name="ost_kolicina1" id='ost_kolicina19'
                                   onchange="calculate_total_ot();"
                                   value="<?php if (isset($putt)) echo $putt['ost_kolicina1']; else echo ''; ?>"
                                   placeholder='Unesite količinu' class="form-control">

                        </div>
                        <div class="col-sm-3">
                            <label>Iznos KM:</label>
                            <input step="any" type="number" min="0" name="ost_iznos1" id='ost_iznos19'
                                   onchange="calculate_total_ot();"
                                   value="<?php if (isset($putt)) echo $putt['ost_iznos1']; else echo ''; ?>"
                                   placeholder='Unesite iznos' class="form-control">
                        </div>
                        <div class="col-sm-3">
                            <label>Ukupno KM:</label>
                        ...
                        for ($i = 2; $i <= 6; $i++) {
                            if ($putt['ost_trosak' . $i]) {
                                ...
                                <div class="col-sm-12 ultimate_class" id="TextBoxDiv<?php echo $i; ?>">
                                    <div class="col-sm-3">
                                        <input type="text" name="ost_trosak<?php echo $i; ?>"
                                               id='ost_trosak<?php echo $i; ?>'
                                               value="<?php if (isset($putt)) echo $putt['ost_trosak' . $i]; else echo ''; ?>"
                                               placeholder='Unesite naziv troška' class="form-control">

                                    </div>
                                    <div class="col-sm-3">
                                        <input step="any" type="number" min="0" name="ost_kolicina<?php echo $i; ?>"
                                               onchange="calculate_total_ot();" id='ost_kolicina<?php echo $i; ?>'
                                               value="<?php if (isset($putt)) echo $putt['ost_kolicina' . $i]; else echo ''; ?>"
                                               placeholder='Unesite količinu' class="form-control">

                                    </div>
                                    <div class="col-sm-3">
                                        <input step="any" type="number" min="0" name="ost_iznos<?php echo $i; ?>"
                                               onchange="calculate_total_ot();" id='ost_iznos<?php echo $i; ?>'
                                               value="<?php if (isset($putt)) echo $putt['ost_iznos' . $i]; else echo ''; ?>"
                                               placeholder='Unesite iznos' class="form-control">
                                    </div>
                                    <div class="col-sm-3">
        ...
        <div class="container row box">
            <div class='head col-sm-12'>
                <div class='naslov_holder'><h4>Kratak opis zadatka (poslova) koji je obavljen na službenom putu</h4>
                </div>
                <div class="box-head-btn">
            ...
            <div class='content' id='c3899'>
                <div class="col-sm-12">
                    <div class="col-sm-12">
                        ...
                        <textarea name="ost_kratkiopis" id='ost_kratkiopis'
                                  value="<?php if (isset($putt)) echo $putt['ost_kratkiopis']; else echo ''; ?>"
                                  class="form-control"></textarea>
                    </div>
                </div>
            </div>
        ...
        <div class="container row box">
            ...
            <div class='content' id='c382'>
                <div class="col-sm-12">
                    <div class="col-sm-12">
                        <label>Navesti specifikaciju nastalih troškova koja sadrži naziv, vrstu i iznos troškova
                            nastalih u svrhu službenog putovanja za koje se prilažu računi:</label>
                        <textarea name="ost_specopis" id='ost_specopis'
                                  value="<?php if (isset($putt)) echo $putt['ost_specopis']; else echo ''; ?>"
                                  class="form-control"></textarea>
                    </div>
                </div>
            </div>
        ...
        <div class='container row box' style='margin-bottom:15px;padding:20px;text-align:center'>
            ...
            <?php if (!$_GET['view']) { ?>
                ...
                <button class="btn btn-red btn-lg" onclick="//return planiraj(event)">Sačuvaj! <i
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" style="font-size: 15px;">
                Odabirom svrhe "Upotreba vlastitog automobila u službene svrhe" će biti uklonjena registracija službenog
                puta na kalendaru za odabrani period.
...
<!-- Modal korekcije -->
<div class="modal fade" id="korekcije_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
     aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
...
<div class="hidden tabledata"><?php
    $data5 = $db->query("Select data from  " . $portal_akontacija . " ");
    $data5 = $data5->fetch();
...
//perform a for loop to add the event handler
    ...
    $("#izdaci_kol1").change(function () {
        calculate_total_nocenja();
    });
    $("#izdaci_iznos1").change(function () {
        calculate_total_nocenja();
    });
    $("#izdaci_kol2").change(function () {
        calculate_total_nocenja();
    });
    $("#izdaci_iznos2").change(function () {
        calculate_total_nocenja();
    });
    $("#izdaci_kol3").change(function () {
        calculate_total_nocenja();
    });
    $("#izdaci_iznos3").change(function () {
        calculate_total_nocenja();
    });
    ...
    function change_datum_akontacije() {
        ...
        let ajaxRequest = $.post("/apoteke-app/modules/default/ajax.php", values, function (data) {
