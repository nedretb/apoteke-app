...
if (isset($_GET['dateOD']) and isset($_GET['dateDO'])){
    ...
    $year_id = $db->query("SELECT TOP 1 id FROM [c0_intranet2_apoteke].[dbo].[hourlyrate_year] where year = $godina_poc and user_id = $user_id");
    $year_id = $year_id->fetch();
    $year_id = $year_id['id'];

    $get_id = $db->query("SELECT top 1 id from [c0_intranet2_apoteke].[dbo].[hourlyrate_day] where day = $dan_poc and month_id = $mjesec_poc and year_id = $year_id");
    $get_id = $get_id->fetch();
    $_GET['id'] = $get_id['id'];
}
...
if($_POST){
    ...
    if(empty($_POST['sl_put_id'])){
        ...
        if(isset($_GET['get_year'])){
            $check_status = $db->query("SELECT count(*) as c FROM [c0_intranet2_apoteke].[dbo].[hourlyrate_day] as h
inner join [c0_intranet2_apoteke].[dbo].[hourlyrate_year] as y on y.user_id = h.user_id
...
employee_no = ".$_POST['employee_no']." and (status not in (83, 5) and corr_status not in (83, 5) or change_req = 1)");
            ...
            $check_status = $db->query("SELECT count(*) as c FROM [c0_intranet2_apoteke].[dbo].[hourlyrate_day] where Date >='".$begin_date."' and  Date <='".$end_date."' and 
employee_no = ".$_POST['employee_no']." and (status not in (83, 5) and corr_status not in (83, 5) or change_req = 1)");
            $check_status = $check_status->fetch();
        }
        //status
        ...
        if($time_poc > strtotime($begin_date)){
            $check_editable = $db->query("SELECT count(*) as c FROM [c0_intranet2_apoteke].[dbo].[hourlyrate_day] where Date >='".$begin_date."' and  Date <='".$end_date."' and 
...
employee_no = ".$_POST['employee_no']." and editable ='N' ");
        ...
        //editing sl put
        $get_sl_put = $db->query("SELECT id,sp.status,sp.request_id,sp.pocetak_datum,sp.kraj_datum,sp.kraj_datum2 FROM [c0_intranet2_apoteke].[dbo].[sl_put] as sp where sp.id='".$_POST['sl_put_id']."'");
        ...
        //check status
        $check_status = $db->query("SELECT count(*) as c FROM [c0_intranet2_apoteke].[dbo].[hourlyrate_day] 
  ...
  employee_no = ".$_POST['employee_no']." and (status not in (83, 5) and corr_status not in (83, 5) or change_req = 1) and id not in (
   SELECT id FROM [c0_intranet2_apoteke].[dbo].[hourlyrate_day] 
   ...
   and employee_no = ".$_POST['employee_no'].")");
        ...
        $check_editable['c'] =0;
        if($time_poc > strtotime($begin_date)){
            $check_editable = $db->query("SELECT count(*) as c FROM [c0_intranet2_apoteke].[dbo].[hourlyrate_day] 
                  where Date >='".$begin_date."' and  Date <='".$end_date."' and 
    ...
    if(strtotime($get_sl_put['pocetak_datum']) != strtotime($_POST['pocetak_datum']) or strtotime($get_sl_put['kraj_datum']) != strtotime($_POST['kraj_datum']) or strtotime($get_sl_put['kraj_datum2']) != strtotime($_POST['kraj_datum2']) or empty($_POST['sl_put_id']) or $get_sl_put['svrha'] != $_POST['svrha']){
        ...
        if ($_POST['svrha'] != 'UPOTREBA VLASTITOG AUTOMOBILA U SLUŽBENE SVRHE'){
          ...
          timest_edit = '".date('Y-m-d h:i:s')."',
            ...
            $novi_put = $db->query("UPDATE [c0_intranet2_apoteke].[dbo].[hourlyrate_day] set 
          status = 
          case 
    ...
    if(true){
        try {
            ...
            if (empty($_GET['sl_put_id'])){
                $data = $db->query("INSERT INTO [c0_intranet2_apoteke].[dbo].[sl_put] ([request_id],[status],[svrha],[pocetak_datum],[pocetak_vrijeme],[kraj_datum],[kraj_vrijeme],[polazna_drzava],[grad_polaska],[odredisna_drzava],[odredisni_grad],[odredisna_drzava2],[odredisni_grad2],[odredisna_drzava3],[odredisni_grad3],[razlog_putovanja],[napomena],[iznos_akontacije],[valuta],[datum_akontacije],[primanje_sredstva],[akontacija_napomena],[vrsta_transporta],[transport_pocetak_datum],[transport_pocetak_vrijeme],[transport_kraj_datum],[transport_kraj_vrijeme],[transport_polazna_drzava],[transport_grad_polaska],[transport_odredisna_drzava],[transport_odredisni_grad],[transport_napomena],[vrsta_smjestaja],[smjestaj_pocetak_datum],[smjestaj_pocetak_vrijeme],[smjestaj_kraj_datum],[smjestaj_kraj_vrijeme],[smjestaj_drzava],[smjestaj_grad],[smjestaj_adresa],[osiguranje],[osiguranje_pocetak_datum],[osiguranje_pocetak_vrijeme],[osiguranje_kraj_datum],[osiguranje_kraj_vrijeme],[dokument_broj],[viza],[osiguranje_napomena],[lock],[status_hr],[created_at],kraj_datum2,kraj_vrijeme2,kategorija_hotela,placeno_biznis_karticom,za_isplatu_povrat,smjestaj_napomena,employee)
    VALUES ("."'".
                    $_POST['request_id'] ."','".
                    $_POST['status'] ."','".
                    $_POST['svrha'] ."','".
    ...
    '".$_POST['employee_no']."'
                ...
                $data_ostalo = $db->query("INSERT INTO [c0_intranet2_apoteke].[dbo].[sl_put_ostali_info] (
        [request_id],
        [trosak1],
        [trosak2],
        [kolicina1],
        ...
        '".$_POST['dnevnica_postotak'] ."'
                ...
                $data_before_updateq = $db->query("select * from [c0_intranet2_apoteke].[dbo].[sl_put] as s join [c0_intranet2_apoteke].[dbo].[sl_put_ostali_info] as i on i.sl_put_id_fk = s.id where id =".$_POST['sl_put_id']);
                $data_before_update = $data_before_updateq->fetch(PDO::FETCH_ASSOC);
                $data_ostalo = $db->query("UPDATE [c0_intranet2_apoteke].[dbo].[sl_put_ostali_info] SET
        ...
        ");
                ...
                $data = $db->query("UPDATE [c0_intranet2_apoteke].[dbo].[sl_put] SET 
            [request_id] ='".$_POST['request_id'] ."'
           ,[status]='".$_POST['status'] ."'
           ,[svrha]='".$_POST['svrha'] ."'
...
//
  ...
  inner join [c0_intranet2_apoteke].[dbo].[users] as table3
  ON table1.user_id = table3.user_id
  left join [c0_intranet2_apoteke].[dbo].[countries] as c on c.country_id = table2.odredisna_drzava
...
//kraj notifikacije o izmjeni
        ...
        if(empty($_GET['sl_put_id'])){
            ...
            if(empty($_GET['sl_put_id'])){
                $new_slput= $db->query("SELECT top(1) id from [c0_intranet2_apoteke].[dbo].[sl_put] order by id desc ");
    ...
    ");
      ...
      ";
        ...
        if (isset($_POST['obrada'])){
            ...
            $last_log = $db->query("Select top(1) * from [c0_intranet2_apoteke].[dbo].[sl_put_logs] where sl_put_request_id = ".$_POST['sl_put_id']." order by vrijeme desc ");
            $last_log = $last_log->fetch();

            if($last_log['operation'] == 'odbijanje'){


                $provjeraq = $db->query("SELECT count(*) as aa FROM [c0_intranet2_apoteke].[dbo].[sl_put] where status_hr = '1' and id =".$_POST['sl_put_id']);
                foreach ($provjeraq as $one){
                    $provjera = $one['aa'];
                }

                if ($one['aa']==0){
                    $odobri_req = $db->query("UPDATE [c0_intranet2_apoteke].[dbo].[sl_put] set status_hr = 1 where id =".$_POST['sl_put_id']);
                    $odobri_req->execute();
                    //logs
    ...
    VALUES (".$_POST['sl_put_id'].", 'odobravanje', $user, $ts)");
                ...
                $odobri_req2 = $db->query("UPDATE [c0_intranet2_apoteke].[dbo].[sl_put] set na_obradi = 1 where id =".$_POST['sl_put_id']);
                $odobri_req2->execute();
...
//sifrarnici
$sifrarnik_svrha= $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[sifrarnici] where active = 1 and name ='svrha'");
$sifrarnik_valuta= $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[sifrarnici] where active = 1 and name ='valuta'");
$sifrarnik_primanje_sredstava= $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[sifrarnici] where active = 1 and name ='primanje_sredstava'");
...
$sifrarnik_vrsta_smjestaja= $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[sifrarnici] where active = 1 and name ='vrsta_smjestaja'");
$sifrarnik_osiguranje= $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[sifrarnici] where active = 1 and name ='osiguranje'");
$sifrarnik_viza= $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[sifrarnici] where active = 1 and name ='viza'");
$cijena_goriva_postotak= $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[sifrarnici] where active = 1 and name ='cijena_goriva_postotak'");

$sifrarnik_drzave = $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[countries] ORDER BY name COLLATE Latin1_General_CS_AS_KS_WS ASC ");
...
if(!empty($_GET['sl_put_id'])){
    $putq = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[sl_put] where id =".$_GET['sl_put_id']);
    $put = $putq->fetch();
} else $put =null;

if(isset($_GET['sl_put_id'])){
    $putb = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[sl_put_ostali_info] where sl_put_id_fk ='".$_GET['sl_put_id']."'");
    $putt = $putb->fetch();
} else $putt = null;

// postavljanje datuma na selektovani
if(isset($_GET['get_year'])){
    $check_status = $db->query("SELECT * FROM [c0_intranet2_apoteke].[dbo].[users] as u 
left join [c0_intranet2_apoteke].[dbo].[hourlyrate_year] as y on y.user_id= u.user_id
left join [c0_intranet2_apoteke].[dbo].[hourlyrate_day] as d on d.user_id= u.user_id
where Date = '".date("Y-m-d", strtotime($_GET['dateOD']))."' and y.id=  ".$_GET['get_year']);
    $hd_data = $check_status->fetch();

}else{
    $hd_data = $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[hourlyrate_day] where id = ".$_GET['id']);
...
while($tip == 'BHOLIDAY' or in_array($dan,[6,7])){
    $id --;
    $dan_data = $db->query("SELECT * from [c0_intranet2_apoteke].[dbo].[hourlyrate_day] where id = $id ");
...
<section class="full">
    ...
    <form method='POST' id='forma_sl_put'>
                ...
                <div class="col-sm-12" >
                    <div class="col-sm-12">
                        <div class="col-sm-3">
                            ...
                            <input type="text" name="trosak1" id='trosak1' value="<?php if(isset($putt)) echo $putt['trosak1']; else echo '';?>" placeholder='Unesite naziv troška' class="form-control">
                    ...
                    <div class="col-sm-12" id="prevoz_red2">
                        <div class="col-sm-3">
                            ...
                            <input type="text" name="trosak2" id='trosak2' value="<?php if(isset($putt)) echo $putt['trosak2']; else echo '';?>" placeholder='Unesite naziv troška' class="form-control">
        ...
        <div class="container row box">
            <div class='head col-sm-12'>
                <div class='naslov_holder'><h4>Postotak na dnevnicu</h4></div>
                <div style="display: none;" class="box-head-btn">
                    <a href="javascript:;" class="ion-ios-arrow-up" data-widget="collapse" data-id="c32841"></a>
                </div>
            </div>
            <div class='content' id='c3284'>
                <div class="col=sm-12">
...
<div class="hidden tabledata"><?php
    $data5 = $db->query("Select data from [c0_intranet2_apoteke].[dbo].[akontacija]");
    $data5 = $data5->fetch();
