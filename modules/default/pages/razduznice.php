            ...
            <div class="row">
                ...
                <div class="col-sm-12" style="margin-top:20px;">
                    ...
                    <?php if ($radnik_potpisao_razduznica) { ?>
                        <span class="potvrda" style="background:#ba0d0d;margin-left:15px;margin-right:15px;">Razdužnica potpisana!</span>
                        <br/>
                    <?php } ?>

                    <?php if ($radnik_odbio_razduznica) { ?>
                        <span class="potvrda" style="background:#ba0d0d;margin-left:15px;margin-right:15px;">Razdužnica odbijena!</span>
                        <br/>
                    <?php } ?>
        ...
        <div class="col-sm-12">
            <table class="alt" id="obuke">
                ...
                foreach ($query as $key => $item) {
                    ...
                    <tr>
                        <td><input readonly="readonly" type="text" class="rcorners1" style="width:300px;" name="oj[]"
                                   value="<?php echo $oj; ?>"></td>
                        ...
                        <td><input readonly="readonly" type="text" class="rcorners1" style="width:60px;"
                                   name="razduzeno[]"
                                   value="<?php echo _transformZaduzeno($razduznice_statusi[$key]['razduzen']); ?>">
                        </td>
                        <td><input readonly="readonly" type="text" class="rcorners1" name="datum[]"
                                   value="<?php if ($razduznice_statusi[$key]['datum'] == '') {
                                       echo '';
                                   } else {
                                       echo date('d/m/Y', strtotime($razduznice_statusi[$key]['datum']));
                                   } ?>"></td>
                        <td><input readonly="readonly" type="text" class="rcorners1" style="width:60px;"
                                   name="ispravno_stanje[]"
                                   value="<?php echo _transformZaduzeno($razduznice_statusi[$key]['ispravno_stanje']); ?>">
                        </td>
                        <td><input readonly="readonly" type="text" class="rcorners1" name="odgovorna_osoba[]"
                                   value="<?php echo _user($razduznice_statusi[$key]['odgovorna_osoba'])['fname'] . ' ' . _user($razduznice_statusi[$key]['odgovorna_osoba'])['lname']; ?>">
                        </td>
                        <td><textarea id="<?php echo 'zapisnik-' . $item['item_id'] . '-' . $usr; ?>"
                                      name="zapisnik[]" <?php if ($razduznice_statusi[$key]['saglasan'] == 0) echo 'style="border-color: red"'; ?> rows="3"
                                      cols="40"
                                      spellcheck="false"><?php echo $razduznice_statusi[$key]['zapisnik']; ?></textarea>
                        </td>
                        <td><select id="<?php echo 'saglasan-' . $item['item_id'] . '-' . $usr; ?>" name="saglasan[]"
                                    class="rcorners1" class="form-control" style="width:60px;outline:none;">
            ...
            <a id="link_potvrda" href="<?php echo $url . '/modules/' . $_mod . '/ajax.php'; ?>"
               style="float:right;display:<?php if ($poslano_radniku_razduznica and !$radnik_potpisao_razduznica) {
                   echo '';
               } else {
                   echo 'none';
               } ?>" class="table-btn alt1" data-widget="send" data-id="radnik_potpisuje_razduznica:<?php echo $usr; ?>"
               ...
               data-response="<?php echo __('Uspješno potpisano!'); ?>"><i class="fa fa-arrow-circle-right"
                                                                           style="float:left;color:#ba0d0d;"></i><label
            ...
            <a id="link_potvrda" href="<?php echo $url . '/modules/' . $_mod . '/ajax.php'; ?>"
               style="float:right;display:<?php if ($poslano_radniku_razduznica and !$radnik_potpisao_razduznica) {
                   ...
                   echo 'none';
               } ?>" class="table-btn alt1" data-widget="send" data-id="radnik_odbija_razduznica:<?php echo $usr; ?>"
               data-text="<?php echo __('Da li ste sigurni da želite odbiti razdužnicu?'); ?>"
               data-response="<?php echo __('Uspješno odbijeno!'); ?>"><i class="fa fa-arrow-circle-right"
                                                                          style="float:left;color:#ba0d0d;"></i><label
                        style="font-size:17px;cursor: pointer;">Odbij razdužnicu</label></a><br><br><br>
        </div>
        <button type="submit"
                style="margin-bottom:40px;width:200px;float:right;height:45px;margin-right:15px;background: #ba0d0d;"
...
//$("select option:not(:selected)").prop("disabled", true);
    ...
    $('select:regex(id, .*saglasan.*)').on('change', function () {
        $.post("<?php echo $url . '/modules/' . $_mod . '/ajax.php'; ?>", {
                request: "change-saglasan_razduznica",
                saglasan: this.value,
                saglasan_id: this.id
            },
    ...
    $('textarea:regex(id, .*zapisnik.*)').on('change', function () {
        ...
        $.post("<?php echo $url . '/modules/' . $_mod . '/ajax.php'; ?>", {
                request: "change-razduznica-zapisnik",
                zapisnik: this.value,
                zapisnik_id: this.id
            },
            function (returnedData) {
