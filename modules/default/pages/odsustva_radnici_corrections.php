...
$get_month = $db->query("SELECT * FROM  " . $portal_hourlyrate_month_correctoins . "  WHERE id = " . $mjesec);

$get_y = $db->query("SELECT count(*) FROM  " . $portal_hourlyrate_year . "  WHERE id='" . $godina . "'");
$get_m = $db->query("SELECT count(*) FROM  " . $portal_hourlyrate_month_correctoins . "  WHERE id='" . $mjesec . "'");
...
if ($total > 0 || $total2 > 0) {
    ...
    if (!isset($_POST['dateTo'])) {
        ...
        $month_from = $mjesec;
        $month_to = $mjesec;
        $datumOD = date('d/m/Y', strtotime("first day of -1 month"));
        $datumDO = date('d/m/Y', strtotime("last day of -1 month"));
    }
                    ...
                    <div class="row col-sm-12" style="margin-left:-2%; display: flex; justify-content: space-evenly;">
                        <div class="col-sm-1" style="padding-right:0px;">
                            <input type="text" name="dateFrom" class="form-control" style="height:35px;" id="dateOD1"
                                ...
                                echo date('d.m.Y', strtotime("first day of -1 month"));
                            } elseif (count($_GET) == 2) {
                        ...
                        <div class="col-sm-2" style="">
                            <button class="btn btn-allow" id="accept_all"
                                    style="width:150px; height:34px; text-align:left;"><?php echo __('Odobri izostanke'); ?>
                                <i class="fa fa-check" style="padding-top:3px;" aria-hidden="true"></i></button>
                        </div>
