...
if (isset($_POST['request'])) {
   ...
   and  [year]='" . $absence_year . "'");
    ...
    if ($_POST['request'] == 'parent-day-add_apsolute' or ($_POST['request'] == 'day-edit' and $_POST['status'] == 67)) {
        ...
        if ((($weekends_start >= 6 and $weekends_start <= 7) and ($weekends_end >= 6 and $weekends_end <= 7)) and !in_array($_POST['status'], array(43, 44, 45, 61, 62, 65, 67, 68, 69, 74, 75, 76, 77, 78, 73, 81, 107, 108))) {
  ...
  ");
   ...
   and year_id=" . $getYear);
	...
	AND weekday<>'6' AND weekday<>'7' AND (date_NAV_corrections is null)");
        ...
        $askedgo = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "   where 
  ...
  or (corr_status='32') or (corr_status='79'))");
        $currpcm = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='21')");
        $upcm = $db->query("SELECT * FROM  " . $portal_vacation_statistics . "  WHERE employee_no='" . $empid . "' and year = '$godina'");
        $currupcm = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='22')");

        $curruP_1 = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='27')");
        $curruP_2 = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='28')");
        $curruP_3 = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='29')");
        $curruP_4 = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='30')");
        $curruP_5 = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='31')");
        $curruP_6 = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='32')");
        $curruP_7 = $db->query("SELECT sum(" . $br_sati . ") as sum_hour FROM  " . $portal_hourlyrate_day . "  WHERE year_id='" . $getYear . "' AND weekday<>'6' AND weekday<>'7' AND (corr_status='79')");
    ...
    ";
        ...
        if ($mail_settings['hr_notifications'] == '1') {
            if (in_array($_POST['status'], $array_bolovanje)) {
                ...
                $mail->setFrom("test.hr@raiffeisengroup.ba", "Infodom");

                //$mail->addAddress($_user['email']);
                //$mail->addAddress($mail_settings['hr_supportt_mail']);
                $mail->addAddress("infodom.test@raiffeisengroup.ba");
                //
        ...
        /* if($countHol>0  and ($status != '67' and $status != '73' and $status != '81'))
 echo '<div class="alert alert-danger text-center">'.__('Upozorenje : Državni praznici većeg prioriteta nisu ažurirani!').'</div>'; */
    ...
    if ($_POST['request'] == 'day-edit' and $_POST['status'] != '67') {
        ...
        $get_old_status = $db->query("SELECT status, KindOfDay, weekday FROM  " . $portal_hourlyrate_day . "  WHERE id='" . $this_id . "'  ");
  ...
  AND weekday<>'6' AND weekday<>'7' AND (corr_status='32')");
     ...
     and weekday<>'6' AND weekday<>'7' AND (corr_status='79')");
        ...
        if ($old_status['weekday'] == '6' or $old_status['weekday'] == '7') {
            if (!in_array($_POST['status'], array(5, 43, 44, 45, 61, 62, 65, 67, 68, 69, 73, 75, 76, 77, 78, 81, 85, 86, 87, 88, 89, 90, 105, 107, 108, 91, 92, 93, 94, 95, 96))) {
                echo '<div class="alert alert-danger"><b>' . __('Greška!') . '</b><br/>' . __('Ne možete registrovati vikendom!') . '</div>';
                return;
            }
        }
        ...
        if ((in_array($_POST['status'], array(85, 86, 87, 88, 89, 90)) or (@$_POST['hour_pre'] != '' and @$_POST['hour_pre'] != '0')) or ($old_status['weekday'] == '6' or $old_status['weekday'] == '7')) {
            if (in_array($_POST['status'], array(85, 86, 87, 88, 89, 90, 5))) {
                $rev_status = 1;
        ...
        if ($old_status['weekday'] == '6' or $old_status['weekday'] == '7') {
            $add_review_user = "review_user = '$_user[user_id]',";
        } else {
            $add_review_user = "";
        }
    ...
    if ($_POST['request'] == 'parent-day-cancel_apsolute') {
        ...
        $get_count1 = $db->query("SELECT count(*) as countOdobreno FROM  " . $portal_hourlyrate_day . "  WHERE (corr_review_status='1') and (
   (day >= " . $day_from . " and month_id = " . $month_from . " and (" . $month_from . " <> " . $month_to . "))  OR
   (day <= " . $day_to . " and month_id = " . $month_to . " and (" . $month_to . " <> " . $month_from . ")) OR
   ((day >= " . $day_from . " and day <= " . $day_to . ") and month_id = " . $month_from . " and month_id = " . $month_to . ") OR
   (month_id > " . $month_from . " and month_id < " . $month_to . ")
   ...
   and year_id=" . $getYear);
        ...
        $get_count2 = $db->query("SELECT count(*) as not_redovni FROM  " . $portal_hourlyrate_day . "  WHERE ((corr_status='5' and hour != '0' and (hour_pre IS NOT NULL or hour_pre > 0)) or (corr_status != '5')) and (
   (day >= " . $day_from . " and month_id = " . $month_from . " and (" . $month_from . " <> " . $month_to . "))  OR
   (day <= " . $day_to . " and month_id = " . $month_to . " and (" . $month_to . " <> " . $month_from . ")) OR
   ((day >= " . $day_from . " and day <= " . $day_to . ") and month_id = " . $month_from . " and month_id = " . $month_to . ") OR
   (month_id > " . $month_from . " and month_id < " . $month_to . ")
   ...
   and year_id=" . $getYear);
        ...
        $get_statuses_redovan = $db->query("SELECT corr_status,weekday,hour,hour_pre FROM  " . $portal_hourlyrate_day . "  WHERE weekday not in (6,7) and (
	   (day >= " . $day_from . " and month_id = " . $month_from . " and (" . $month_from . " <> " . $month_to . "))  OR
	   (day <= " . $day_to . " and month_id = " . $month_to . " and (" . $month_to . " <> " . $month_from . ")) OR
	   ((day >= " . $day_from . " and day <= " . $day_to . ") and month_id = " . $month_from . " and month_id = " . $month_to . ") OR
	   (month_id > " . $month_from . " and month_id < " . $month_to . ")
	   ...
	   and year_id=" . $getYear);
        ...
        foreach ($fetch_statuses_redovan as $k => $v) {
            if ($v['corr_status'] != '5' or ($v['corr_status'] == '5' and $v['hour'] != '0' and ($v['weekday'] == '6' or $v['weekday'] == '7')) or ($v['corr_status'] == '5' and $v['hour'] != '0' and $v['hour_pre'] > 0)) {
                $samo_redovni = 0;
            }
        }
        ...
        if ($notRedovni == 0) {
            echo '<div class="alert alert-danger text-center"><b>' . __('Upozorenje!') . '</b><br/>' . __('Nemate šta otkazati.') . '</div>';
            return;
        }

        $yearq = $db->query("select * from  " . $portal_hourlyrate_year . "  where id = $getYear");
        $yearq = $yearq->fetch();

        $get_statuses = $db->query("SELECT * FROM  " . $portal_hourlyrate_day . "  WHERE year_id = $getYear and Date between '" . $yearq['year'] . '-' . $month_from . '-' . $day_from . "' and '" . $yearq['year'] . '-' . $month_to . '-' . $day_to . "'  ");
   ...
   and year_id=?
    ...
    if ($_POST['request'] == 'day-review') {
        ...
        if ($_POST['status'] == '2') {
            $status_id = '5';
            $additional_query = ", hour_pre = NULL,
        corr_pre = NULL,
        status_bh = NULL,
        hour = $_user[br_sati]";
            $status = '0';
        ...
        if ($hours_no == '0') {
            echo '<div class="alert alert-danger text-center">' . __('Ne možete ažurirati registraciju od 0h!') . '</div>';
            exit;
        }

        if (($weekday == '6' or $weekday == '7') and !in_array($_POST['status_id'], array(43, 44, 45, 61, 62, 65, 67, 68, 69, 73, 74, 75, 76, 77, 78, 81, 105, 107, 108, 116, 123, 124, 125, 126, 127, 129, 130, 131, 132, 133, 134, 135, 136, 137))) {
            echo '<div class="alert alert-danger text-center"><b>' . __('Upozorenje!') . '</b><br/>' . __('Ne možete ažurirati ovu registraciju.') . '</div>';
            return;
        }
    ...
    if ($_POST['request'] == 'day-review_apsolute') {
        ...
        if ($_POST['status'] == '2') {
            ...
            $data = "UPDATE  " . $portal_hourlyrate_day . "  SET
      corr_review_status = case when [KindOfDay]='BHOLIDAY' then 1 else 0 end,
      review_comment = ?,
      review_user = ?,
   ...
   and hour != '0'
   and status != 5
   and id = case when ([weekday] = '6' or [weekday] = '7') and [corr_status] not in (43, 44, 45, 61, 62, 65, 67, 68, 69, 73, 74, 75, 76, 77, 78, 81, 105, 107, 108, 116, 123, 124, 125, 126, 127, 129, 130, 131, 132, 133, 134, 135, 136, 137) then 0 else [id] end
    ";
        ...
        if ($res->rowCount() >= 1) {
            ...
            echo '<div class="alert alert-danger text-center">' . __('Registracije sa 0h i nedozvoljene registracije nisu odobrene!') . '</div>';
        }
