  ...
  AND weekday<>'6' AND weekday<>'7' AND (status='30') AND (date_NAV is null)");
   ...
   and  [year]='" . $absence_year . "'");
    ...
    if ($_POST['request'] == 'get-organization') {
        ...
        //filter data
        $get_data = $db->query("
    select cl.[Employee No_],cl.[Department Cat_ Description],cl.[Sector Description],cl.[Group Description],cl.[Team Description],e.[Termination Date],e.[First Name] +' ' + e.[Last Name] as ime
from  " . $nav_employee . "  as e
  ...
  where cl.Active=1 and cl.[Show Record]=1 and (" . $_user['employee_no'] . " in (admin1,admin2,admin3,admin4,admin5,admin6,admin7,admin8) or parent = " . $_user['employee_no'] . " or parent2 = " . $_user['employee_no'] . " or  " . $_user['employee_no'] . " in (parentMBO2,parentMBO3,parentMBO4,parentMBO5,parentMBO6,MBO))
  ");
        ...
        $sektori = unique_multidim_array($data, 'Sector Description');
        array_multisort(array_column($sektori, "Sector Description"), SORT_ASC, $sektori);
        foreach ($sektori as $key => $value) {
            if ($value['Sector Description'] == $_POST['sektor']) $sel = 'selected';
            else $sel = '';

            if ($value['Sector Description'] != '')
                $options['sektor'] .= '<option value="' . $value['Sector Description'] . '" ' . $sel . '>' . $value['Sector Description'] . '</option>';
        }
        if ($options['sektor'] == '') $options['sektor'] = '<option value="">' . __('Nema unesenih opcija') . '</option>';
        if (count($sektori) > 1) $options['sektor'] = '<option value="">' . __('Odaberi...') . '</option>' . $options['sektor'];

        $odjeli = unique_multidim_array($data, 'Department Cat_ Description');
        foreach ($odjeli as $key => $value) {
            if ($value['Department Cat_ Description'] == $_POST['odjel']) $sel = 'selected';
            else $sel = '';

            if ($value['Department Cat_ Description'] != '') {
                if (!empty($_POST['sektor'])) {
                    if ($value['Sector Description'] == $_POST['sektor']) {
                        $options['odjel'] .= '<option value="' . $value['Department Cat_ Description'] . '" ' . $sel . '>' . $value['Department Cat_ Description'] . '</option>';
                    }
                }
                if (empty($_POST['tim']) and empty($_POST['grupa']) and empty($_POST['odjel']) and empty($_POST['sektor'])) {
                    $options['odjel'] .= '<option value="' . $value['Department Cat_ Description'] . '" ' . $sel . '>' . $value['Department Cat_ Description'] . '</option>';
                }
            }
        }
        if ($options['odjel'] == '') $options['odjel'] = '<option value="">' . __('Nema unesenih opcija') . '</option>';
        if (count($odjeli) > 1) $options['odjel'] = '<option value="">' . __('Odaberi...') . '</option>' . $options['odjel'];

        $grupe = unique_multidim_array($data, 'Group Description');
        array_multisort(array_column($grupe, "Group Description"), SORT_ASC, $grupe);
        foreach ($grupe as $key => $value) {
            if ($value['Group Description'] == $_POST['grupa']) $sel = 'selected';
            else $sel = '';

            if ($value['Group Description'] != '') {
                if (!empty($_POST['odjel'])) {
                    if ($value['Department Cat_ Description'] == $_POST['odjel']) {
                        $options['grupa'] .= '<option value="' . $value['Group Description'] . '" ' . $sel . '>' . $value['Group Description'] . '</option>';
                    }
                }
                if (empty($_POST['tim']) and empty($_POST['grupa']) and empty($_POST['odjel']) and empty($_POST['sektor'])) {
                    $options['grupa'] .= '<option value="' . $value['Group Description'] . '" ' . $sel . '>' . $value['Group Description'] . '</option>';
        ...
        if ($options['grupa'] == '') $options['grupa'] = '<option value="">' . __('Nema unesenih opcija') . '</option>';
        if (count($grupe) > 1) $options['grupa'] = '<option value="">' . __('Odaberi...') . '</option>' . $options['grupa'];

        $timovi = unique_multidim_array($data, 'Team Description');
        array_multisort(array_column($timovi, "Team Description"), SORT_ASC, $timovi);
        foreach ($timovi as $key => $value) {
            if ($value['Team Description'] == $_POST['tim']) $sel = 'selected';
            else $sel = '';

            if ($value['Team Description'] != '') {
                if (!empty($_POST['grupa'])) {
                    if ($value['Group Description'] == $_POST['grupa']) {
                        $options['tim'] .= '<option value="' . $value['Team Description'] . '" ' . $sel . '>' . $value['Team Description'] . '</option>';
                    }
                }
                if (empty($_POST['tim']) and empty($_POST['grupa']) and empty($_POST['odjel']) and empty($_POST['sektor'])) {
                    $options['tim'] .= '<option value="' . $value['Team Description'] . '" ' . $sel . '>' . $value['Team Description'] . '</option>';
        ...
        if ($options['tim'] == '') $options['tim'] = '<option value="">' . __('Nema unesenih opcija') . '</option>';
        if (count($timovi) > 1) $options['tim'] = '<option value="">' . __('Odaberi...') . '</option>' . $options['tim'];

        $zaposlenici = unique_multidim_array($data, 'Employee No_');
        array_multisort(array_column($zaposlenici, "ime"), SORT_ASC, $zaposlenici);

        foreach ($zaposlenici as $key => $value) {
            if ($value['Employee No_'] == $_POST['zaposlenik']) $sel = 'selected';
            else $sel = '';

            if (!empty($_POST['tim'])) {
                if ($value['Team Description'] == $_POST['tim']) {
                    $options['zaposlenik'] .= '<option value="' . $value['Employee No_'] . '" ' . $sel . '>' . $value['ime'] . '</option>';
                }
            } else if (!empty($_POST['grupa'])) {
                if ($value['Group Description'] == $_POST['grupa']) {
                    $options['zaposlenik'] .= '<option value="' . $value['Employee No_'] . '" ' . $sel . '>' . $value['ime'] . '</option>';
                }
            } else if (!empty($_POST['odjel'])) {
                if ($value['Department Cat_ Description'] == $_POST['odjel']) {
                    $options['zaposlenik'] .= '<option value="' . $value['Employee No_'] . '" ' . $sel . '>' . $value['ime'] . '</option>';
                }
            } else if (!empty($_POST['sektor'])) {
                if ($value['Sector Description'] == $_POST['sektor']) {
                    $options['zaposlenik'] .= '<option value="' . $value['Employee No_'] . '" ' . $sel . '>' . $value['ime'] . '</option>';
                }
            }
        ...
        if ($options['zaposlenik'] == '') $options['zaposlenik'] = '<option value="">' . __('Nema unesenih opcija') . '</option>';
        if (count($zaposlenici) > 1) $options['zaposlenik'] = '<option value="">' . __('Odaberi...') . '</option>' . $options['zaposlenik'];
