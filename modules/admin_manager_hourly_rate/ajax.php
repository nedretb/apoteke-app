...
//Upis pocetka bolovanja
   ...
   and [Date] >='" . $user_edit['employment_date'] . "'";
        ...
        foreach ($get_mail_fetch as $key => $value) {
            $mail_settings[$value['name']] = $value['value'];
                                                                                ...
                                                                                //116, 123, 124, 125, 126, 127, 129, 130, 131, 132, 133, 134, 135, 136, 137
  ...
  AND weekday<>'6' AND weekday<>'7' AND (status='32') AND (date_NAV is null)");
     ...
     and weekday<>'6' AND weekday<>'7' AND (status='79') AND (date_NAV is null)");
        ...
        if (($_POST['status'] == '19') and (($propaloGO == 1) or ($filter_month > 6))) {
            echo '<div class="alert alert-danger"><b>' . __('Greška!') . '</b><br/>' . __('Nemate pravo na godišnji iz predhodne godine!') . '</div>';
            return;
        }
   ...
   and year_id=" . $getYear);
        ...
        $get_statuses_redovan = $db->query("SELECT status,weekday,hour,hour_pre FROM  " . $portal_hourlyrate_day . "  WHERE  (
	   (day >= " . $day_from . " and month_id = " . $month_from . " and (" . $month_from . " <> " . $month_to . "))  OR
	   (day <= " . $day_to . " and month_id = " . $month_to . " and (" . $month_to . " <> " . $month_from . ")) OR
	   ((day >= " . $day_from . " and day <= " . $day_to . ") and month_id = " . $month_from . " and month_id = " . $month_to . ") OR
	   (month_id > " . $month_from . " and month_id < " . $month_to . ")
...
join c0_intranet2_apoteke.dbo.hourlyrate_year as y on u.user_id = y.user_id where y.id = $getYear) end,
   ...
   and year_id=?
    ...
    if ($_POST['request'] == 'day-review') {
        ...
        $check = $db->query("SELECT user_id, year_id,month_id,employee_no,weekday,hour,KindofDay FROM  " . $portal_hourlyrate_day . "  WHERE id='" . $this_id . "' ");


        foreach ($check as $checkvalue) {
            $weekday = $checkvalue['weekday'];
            $hours_no = $checkvalue['hour'];
            $KindofDay = $checkvalue['KindofDay'];
        }
    ...
    hour = $_user[br_sati]";
        ...
        if ($_POST['status_id'] == '5' && $weekday != 6 && $weekday != 7 && empty($_POST['reg'])) {
        ...
        if ($status == 1) {
            ...
            $rev_user = null;
            $corr_status_query = ", corr_status = '$status_id' ";
            $status_corr = '0';
        }
   ...
   and hour != '0'
   and status != 5
   and id = case when ([weekday] = '6' or [weekday] = '7') and [status] not in (43, 44, 45, 61, 62, 65, 67, 68, 69, 73, 74, 75, 76, 77, 78, 81, 105, 107, 108, 116, 123, 124, 125, 126, 127, 129, 130, 131, 132, 133, 134, 135, 136, 137) then 0 else [id] end
    ";
        ...
        if ($res->rowCount() >= 1) {
            ...
            echo '<div class="alert alert-danger text-center">' . __('Registracije sa 0h, redovan rad i nedozvoljene registracije nisu odobrene!') . '</div>';
        }
