...
function PasswordWidget(divid,pwdname)
{
	...
	this.showing_pwd=1;
	this.txtShow = 'Verstecken';
	this.txtMask = 'Anzeigen';
...
function _MakePWDWidget()
{
  ...
  var pwdname = this.pwdobjname;
	...
	code += "<input type='password' class='form-control' name='"+pwdname+"' id='"+this.pwdfieldid+"' "+req+">";

	this.pwdtxtfield=pwdname+"_text";

	this.pwdtxtfieldid = this.pwdtxtfield+"_id";

	code += "<input type='text' class='form-control' name='"+this.pwdtxtfield+"' id='"+this.pwdtxtfieldid+"' style='display: none;' "+req+">";
	...
	code += "<div class='progress' id='"+this.pwdstrengthdiv+"'>";

	this.pwdstrengthbar = pwdname + "_strength_bar";

	code += "<div class='progress-bar' id='"+this.pwdstrengthbar+"'></div>";
...
function passwordStrength()
{
	...
	{
		strengthdiv.style.width=(score)*20+'%';
	}
